...
fn bench_message_serialization(c: &mut Criterion) {
    ...
    group.bench_function("serialize_auth_credentials", |b| {
        ...
        b.iter(|| {
            black_box(serde_json::to_string(&credentials).unwrap())
        })
    });
...
fn bench_session_operations(c: &mut Criterion) {
    ...
    group.bench_function("validate_session", |b| {
        ...
        b.iter(|| {
            let is_valid = !session.session_id.is_empty() && 
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    group.bench_function("large_message_creation", |b| {
        b.iter(|| {
            let mut metadata = std::collections::HashMap::new();
            for i in 0..1000 {
                metadata.insert(format!("key_{}", i), format!("value_with_long_content_{}", i));
            }
