...
use universal_patterns::traits::{Credentials, AuthResult, Principal};
...
fn bench_authorization(c: &mut Criterion) {
    ...
    group.bench_function("authorization_multiple_roles", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            black_box(security.authorize(principal.clone(), "admin").await.unwrap())
        })
    });
...
fn bench_token_operations(c: &mut Criterion) {
    ...
    group.bench_function("token_refresh", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            black_box(security.refresh_token("benchmark_token").await.unwrap())
        })
    });
    ...
    group.bench_function("token_revocation", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            black_box(security.revoke_token("benchmark_token").await.unwrap())
        })
    });
            ...
            for i in 0..50 {
                ...
                match i % 3 {
                    ...
                    1 => {
                        ...
                        handles.push(tokio::spawn(async move {
                            security_clone.authorize(principal, "read").await.map(|_| ())
                        }));
                    }
                    2 => {
                        // Token validation
