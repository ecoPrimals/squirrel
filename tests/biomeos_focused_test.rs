...
async fn test_ai_intelligence_functionality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ai_intelligence.analyze_ecosystem().await?;
...
async fn test_context_state_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    context_state
        .update_session_context("test-session-001", updates)
        .await?;
...
async fn test_mcp_integration_functionality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let session_id = mcp_integration
        .create_coordination_session(participants, "security_check".to_string())
        .await?;
...
async fn test_ecosystem_client_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = config::AppConfig::from_env().unwrap_or_default();
    let custom_client = EcosystemClient::with_config(config);
    assert!(!custom_client.songbird_url.is_empty());
