...
async fn demo_mcp_adapter() -> CommandResult<()> {
    ...
    match adapter.execute_with_auth("admin-cmd", vec![], user_auth).await {
        Ok(result) => println!("Unexpected success: {}", result),
        Err(e) => println!("Expected error: {}", e),
    }
    
    // Try to execute admin command anonymously
    match adapter.execute_with_auth("admin-cmd", vec![], anon_auth).await {
        Ok(result) => println!("Unexpected success: {}", result),
        Err(e) => println!("Expected error: {}", e),
    }
...
async fn demo_adapter_trait() -> CommandResult<()> {
    ...
    registry.register_command(Arc::new(test_cmd.clone()))?;
    mcp.register_command(Arc::new(test_cmd.clone()))?;
    plugin.register_command(Arc::new(test_cmd))?;
