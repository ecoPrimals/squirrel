...
use adapter_pattern_tests::{
    Command, CommandAdapter, CommandError, CommandResult, 
    McpAdapter, TestCommand, Auth, UserRole
};
use std::sync::Arc;
...
async fn main() -> CommandResult<()> {
    ...
    // Create and configure the MCP adapter with enhanced features
...
fn setup_enhanced_mcp_adapter() -> CommandResult<McpAdapter> {
    ...
    adapter.register_command(Arc::new(power_cmd))?;
    adapter.register_command(Arc::new(secure_cmd))?;
    ...
    adapter.add_command_with_permissions("power-tool", vec![UserRole::PowerUser, UserRole::Admin]);
    adapter.add_command_with_permissions("secure-data", vec![UserRole::RegularUser, UserRole::PowerUser, UserRole::Admin]);
    
    // Update user roles
    let mut user_roles = std::collections::HashMap::new();
    user_roles.insert("power".to_string(), vec![UserRole::PowerUser]);
    adapter.user_roles = user_roles;
...
async fn demo_authentication(adapter: &mut McpAdapter) -> CommandResult<()> {
    ...
    match adapter.execute_with_auth("hello", vec![], invalid_auth).await {
...
async fn demo_authorization(adapter: &mut McpAdapter) -> CommandResult<()> {
    ...
    match adapter.execute_with_auth("admin-stats", vec![], admin_auth.clone()).await {
        Ok(result) => println!("  Admin access: {}", result),
        Err(e) => println!("  Admin access error: {}", e),
    }
    
    match adapter.execute_with_auth("admin-stats", vec![], power_auth.clone()).await {
        Ok(result) => println!("  Power user access: {}", result),
        Err(e) => println!("  Power user access error: {}", e),
    }
    
    println!("\n3. Power user command access:");
    match adapter.execute_with_auth("power-tool", vec![], power_auth.clone()).await {
        Ok(result) => println!("  Power user access: {}", result),
        Err(e) => println!("  Power user access error: {}", e),
    }
    
    match adapter.execute_with_auth("power-tool", vec![], regular_auth.clone()).await {
...
async fn demo_token_authentication(adapter: &mut McpAdapter) -> CommandResult<()> {
    ...
    match adapter.execute_with_auth("admin-stats", vec![], token_auth.clone()).await {
    ...
    match adapter.execute_with_auth("hello", vec![], invalid_token).await {
        Ok(result) => println!("  Unexpected success: {}", result),
        Err(e) => println!("  Expected error: {}", e),
    }
...
async fn demo_audit_logging(adapter: &mut McpAdapter) -> CommandResult<()> {
    ...
    // Successful commands
    let _ = adapter.execute_with_auth("hello", vec![], admin_auth.clone()).await?;
    let _ = adapter.execute_with_auth("echo", vec!["test".to_string()], power_auth.clone()).await?;
    
    // Failed command (intentional)
    let _ = adapter.execute_with_auth("nonexistent", vec![], admin_auth.clone()).await.unwrap_err();
    
    println!("\nAudit log entries:");
    for (i, entry) in adapter.get_command_logs().iter().enumerate() {
