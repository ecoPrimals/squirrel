...
impl CommandSuggestions {
    ...
    pub fn add_command_description(
        &self,
    ...
    pub fn get_suggestions(
    ...
    ) -> SuggestionResult<Vec<CommandSuggestion>> {
        // Get command history
        ...
        for command in command_set {
            ...
            if score.meets_threshold(self.confidence_threshold) {
                ...
                // Create suggestion
                let suggestion =
                    CommandSuggestion::new(command, common_args, usage_hint, description, score);
    ...
    pub fn update_patterns(&self) -> SuggestionResult<()> {
        ...
        *shared_patterns = arg_patterns;
        ...
        debug!(
            "Updated suggestion patterns from {} history entries",
            history_entries.len()
        );
        Ok(())
    }
...
mod tests {
    ...
    fn test_context_aware_suggestions() {
        ...
        let has_git_commit = results
            .iter()
            .any(|s| s.command == "git" && s.common_args.contains(&"commit".to_string()));
    ...
    fn test_argument_suggestions() {
        ...
        let has_status = arg_suggestions
            .iter()
            .any(|args| args.contains(&"status".to_string()));
