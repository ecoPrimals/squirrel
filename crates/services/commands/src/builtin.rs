...
impl HelpCommand {
    ...
    pub fn new(registry: Arc<Mutex<CommandRegistry>>) -> Self {
        ...
        if command_help.is_empty() {
            warn!(
                "HelpCommand: Created with empty command information in {:?}",
                duration
            );
        } else {
            info!(
                "HelpCommand: Created with {} commands in {:?}",
                command_help.len(),
                duration
            );
        }
...
impl Command for HelpCommand {
    ...
    fn execute(&self, args: &[String]) -> CommandResult<String> {
        ...
        let result = if args.is_empty() {
            ...
            debug!(
                "HelpCommand: Displaying help for all {} commands",
                self.command_help.len()
            );
            ...
            debug!(
                "HelpCommand: Looking up help for specific command: {}",
                command_name
            );
            ...
            // Command not found
            let err_msg = format!("Command '{command_name}' not found");
...
impl Command for VersionCommand {
    ...
    fn parser(&self) -> clap::Command {
        // Using static strings rather than self methods to avoid lifetime issues
        clap::Command::new("version").about("Shows the current version of the application")
    }
...
impl Command for ExitCommand {
    ...
    fn parser(&self) -> clap::Command {
        clap::Command::new("exit").about("Exits the application")
    }
...
impl Command for KillCommand {
    ...
    fn execute(&self, args: &[String]) -> CommandResult<String> {
        ...
        Ok(format!("Process with PID {pid} terminated"))
    }
...
impl Command for HistoryCommand {
    ...
    fn execute(&self, args: &[String]) -> CommandResult<String> {
        ...
        let matches = self
            ...
            .map_err(|e| CommandError::ExecutionError(format!("Invalid arguments: {e}")))?;
        ...
        if let Some(days) = matches.get_one::<u64>("cleanup") {
            ...
            return Ok(format!(
                "Removed {removed} history entries older than {days} days."
            ));
        }
