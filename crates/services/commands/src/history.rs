...
impl CommandHistory {
    ...
    pub fn with_options(max_size: usize, history_file: impl AsRef<Path>) -> HistoryResult<Self> {
        ...
        if !history_file.exists() {
            ...
            File::create(&history_file).map_err(|err| {
                CommandError::ResourceError(format!("Failed to create history file: {err}"))
            })?;
        }
    ...
    pub fn search(&self, query: &str) -> HistoryResult<Vec<HistoryEntry>> {
        ...
        let results: Vec<HistoryEntry> = entries
            .iter()
            .filter(|entry| {
                entry.command.contains(query) || entry.args.iter().any(|arg| arg.contains(query))
            })
            .cloned()
            .collect();
    ...
    pub fn get_last(&self, count: usize) -> HistoryResult<Vec<HistoryEntry>> {
        ...
        let result: Vec<HistoryEntry> = entries.iter().take(count).cloned().collect();
    ...
    pub fn clear(&self) -> HistoryResult<()> {
        ...
        let _lock = self.file_lock.lock().map_err(|err| {
            CommandError::ResourceError(format!("Failed to acquire file lock: {err}"))
        })?;
        ...
        file.set_len(0).map_err(|err| {
            CommandError::ResourceError(format!("Failed to truncate history file: {err}"))
        })?;
        ...
        match serde_json::from_reader::<_, Vec<HistoryEntry>>(reader) {
            ...
            Err(err) => {
                ...
                let file = File::open(&self.history_file).map_err(|err| {
                    CommandError::ResourceError(format!("Failed to reopen history file: {err}"))
                })?;
    ...
    fn save_entry(&self, entry: &HistoryEntry) -> HistoryResult<()> {
        ...
        let file_metadata = std::fs::metadata(&self.history_file).map_err(|err| {
            CommandError::ResourceError(format!("Failed to get history file metadata: {err}"))
        })?;
...
impl Default for CommandHistory {
    fn default() -> Self {
        // For default instance, use a temporary directory for history file
        // This provides a safe fallback when standard directory access fails
        let temp_history_file = std::env::temp_dir().join("squirrel_command_history.json");

        // Attempt to create with standard config first, fall back to minimal config
        Self::with_options(1000, &temp_history_file).unwrap_or_else(|e| {
            tracing::warn!(
                "Failed to create default command history: {}. Using in-memory only.",
                e
            );
            // Create minimal in-memory only instance
            Self {
                entries: Arc::new(RwLock::new(VecDeque::new())),
                max_size: 100, // Smaller default for fallback
