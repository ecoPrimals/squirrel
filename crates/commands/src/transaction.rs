...
impl CommandTransaction {
    ...
    pub fn add_command(&mut self, command: Box<dyn Command>, args: Vec<String>) -> Result<(), TransactionError> {
    ...
    pub fn add_rollback_handler(&mut self, command_name: &str, handler: Arc<dyn RollbackHandler>) -> Result<(), TransactionError> {
        if self.state != TransactionState::Initialized {
            return Err(TransactionError::InvalidState(format!(
        ...
        self.rollback_handlers.insert(command_name.to_string(), handler);
        Ok(())
    }
    ...
    pub async fn execute(&mut self) -> Result<Vec<String>, TransactionError> {
        ...
        for (idx, (command, args)) in self.commands.iter().enumerate() {
            ...
            if result.is_err() {
                ...
                if let Err(err) = self.rollback().await {
                    ...
                    return Err(TransactionError::ExecutionError(format!(
                        "Command {} failed and rollback failed: {}",
                        command_name, err
                    )));
                }
            ...
            let output = result.as_ref().unwrap().to_string();
            outputs.push(output);
...
mod tests {
    ...
    impl Command for MockCommand {
        ...
        fn execute(&self, _args: &[String]) -> CommandResult<String> {
            if self.success {
                ...
                Err(CommandError::ExecutionError(format!("{} failed", self.name)))
            }
        }
        
        fn parser(&self) -> clap::Command {
            clap::Command::new("mock-command")
    ...
    async fn test_transaction_manager() {
        ...
        // Create transactions
        let transaction1 = manager.create_transaction().unwrap();
        let transaction2 = manager.create_transaction().unwrap();
        
        // Get transaction IDs
        let id1 = transaction1.lock().unwrap().id();
        let id2 = transaction2.lock().unwrap().id();
        ...
        manager.remove_transaction(id1).unwrap();
