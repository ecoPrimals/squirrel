...
impl Clone for JournalError {
    fn clone(&self) -> Self {
        match self {
            ...
            Self::SerializationError(_) => Self::SerializationError(serde_json::from_str::<serde_json::Value>("{}").unwrap_err()),
...
impl CommandJournal {
    ...
    pub fn record_completion(&self, id: String, result: CommandResult<String>) -> JournalResult<()> {
                ...
                for entry in entries.iter_mut() {
                    if entry.id == id {
                        ...
                        if entry.state == JournalEntryState::Recovered {
                            ...
                            info!("Successfully recovered command: {} (ID: {})", command_name, id);
                        } else {
                            report.failed += 1;
...
mod tests {
    ...
    impl Command for TestCommand {
        ...
        fn parser(&self) -> ClapCommand {
            ClapCommand::new("test-command")
                .about("A test command for unit tests")
        }
    ...
    fn test_journal_entry_creation() {
        ...
        assert_eq!(entry.arguments, vec!["arg1".to_string(), "arg2".to_string()]);
    ...
    fn test_journal_entry_complete() {
        ...
        assert_eq!(entry.result, Some("Command executed successfully".to_string()));
        assert!(entry.error.is_none());
    ...
    fn test_journal_entry_fail() {
        ...
        assert!(entry.result.is_none());
        assert_eq!(entry.error, Some("Execution error: Command failed".to_string()));
        assert!(entry.execution_time.is_some());
    }
    ...
    fn test_command_journal_basic_workflow() {
        ...
        journal.record_completion(id.clone(), result.clone()).unwrap();
        ...
        // Verify entry state
        assert_eq!(entry.state, JournalEntryState::Completed);
        assert_eq!(entry.result, Some("Success with args: [\"arg1\", \"arg2\"]".to_string()));
    }
    ...
    fn test_find_incomplete_entries() {
        ...
        let id1 = journal.record_start(&command, &["success".to_string()]).unwrap();
        journal.record_completion(id1, Ok("Success".to_string())).unwrap();
        
        // Create an incomplete entry
        let id2 = journal.record_start(&command, &["incomplete".to_string()]).unwrap();
    ...
    fn test_entry_search() {
        ...
        let id1 = journal.record_start(&command, &["search1".to_string()]).unwrap();
        journal.record_completion(id1.clone(), Ok("Success 1".to_string())).unwrap();
        
        let id2 = journal.record_start(&command, &["search2".to_string()]).unwrap();
        journal.record_completion(id2.clone(), Ok("Success 2".to_string())).unwrap();
        
        let id3 = journal.record_start(&command, &["fail".to_string()]).unwrap();
        journal.record_completion(id3.clone(), Err(CommandError::ExecutionError("Failed".to_string()))).unwrap();
        
        // Search for completed entries
        let completed = journal.search_entries(|entry| entry.state == JournalEntryState::Completed).unwrap();
        assert_eq!(completed.len(), 2);
        
        // Search for failed entries
        let failed = journal.search_entries(|entry| entry.state == JournalEntryState::Failed).unwrap();
        assert_eq!(failed.len(), 1);
        assert_eq!(failed[0].id, id3);
        
        // Search by argument content
        let search2 = journal.search_entries(|entry| 
            entry.arguments.iter().any(|arg| arg.contains("search2"))
        ).unwrap();
    ...
    fn test_journal_capacity() {
        ...
        let id1 = journal.record_start(&command, &["first".to_string()]).unwrap();
        journal.record_completion(id1.clone(), Ok("First success".to_string())).unwrap();
        
        let id2 = journal.record_start(&command, &["second".to_string()]).unwrap();
        journal.record_completion(id2.clone(), Ok("Second success".to_string())).unwrap();
        ...
        let id3 = journal.record_start(&command, &["third".to_string()]).unwrap();
        journal.record_completion(id3.clone(), Ok("Third success".to_string())).unwrap();
