...
use squirrel::session::{Session, SessionManagerImpl, SessionMetadata, SessionConfig, SessionState};
use squirrel::ecosystem::{EcosystemServiceRegistration, EcosystemPrimalType, EcosystemRegistryManager};
use squirrel::ecosystem::registry::{EcosystemRegistryConfig, DiscoveredService, ServiceHealthStatus};
...
async fn test_session_arc_optimization() {
    ...
    assert_eq!(session_arc1.metadata.session_id, session_arc2.metadata.session_id);
}
...
async fn test_ai_provider_string_optimization() {
    ...
    let _response = provider.handle_ai_inference_request(ai_request.clone())
        .await
    ...
    assert!(final_metrics.get("total_operations").unwrap().as_u64() > 
            initial_metrics.get("total_operations").unwrap().as_u64());
    assert!(final_metrics.get("clone_operations_avoided").unwrap().as_u64() >= 1);
}
...
async fn test_zero_copy_metrics_tracking() {
    ...
    for request in ai_requests {
        let _response = provider.handle_ai_inference_request(request)
            .await
            .expect("AI inference failed");
    }
    ...
    assert!(final_metrics.get("clone_operations_avoided").unwrap().as_u64() >= 3);
...
async fn test_context_analysis_optimization() {
    ...
    let _response = provider.handle_context_analysis(context_request)
        .await
...
async fn test_session_management_optimization() {
    ...
    let session_id = create_response.get("session")
    ...
    let _get_response = provider.handle_session_management(get_request)
        .await
...
async fn test_memory_efficiency_under_load() {
    ...
    for _ in 0..num_requests {
        ...
        tasks.push(async move {
            provider_ref.handle_ai_inference_request(request).await
        });
    }
    ...
    assert!(efficiency > 0.8, "Efficiency ratio {} too low under load", efficiency);
}
...
mod performance_tests {
    ...
    async fn benchmark_session_operations() {
        ...
        assert!(retrieval_time < creation_time, 
    ...
    async fn benchmark_provider_selection() {
        ...
        println!("Zero-copy metrics: {}", serde_json::to_string_pretty(&metrics).unwrap());
        
        // Verify optimizations were effective
        let efficiency = metrics.get("efficiency_ratio").unwrap().as_f64();
        assert!(efficiency > 0.9, "Expected high efficiency, got {}", efficiency);
    }
} 