...
async fn chaos_07_memory_pressure() -> ChaosResult<()> {
    ...
    let service = Arc::new(tokio::sync::RwLock::new(MockMemoryAwareService::new(
        "memory-service",
    )));
    ...
    for i in 0..20 {
        ...
        assert!(
            result.is_ok(),
            "Requests should succeed with plenty of memory"
        );
    }
    {
    ...
    {
        let mut svc = service.write().await;
        svc.allocate_mb(601); // 701/1000 = 70.1% > 70% threshold for cache eviction
    ...
    for i in 20..40 {
        ...
        assert!(
            result.is_ok(),
            "Requests should still succeed under moderate pressure"
        );
    }
    {
        let m = metrics.read().await;
        ...
        println!(
            "✅ Phase 2: Cache evictions started - {} evictions",
            m.cache_evictions
        );
    }
    ...
    for i in 50..60 {
        if send_memory_aware_request(&service, &metrics, i)
            .await
            .is_ok()
        {
...
impl MockCpuIntensiveService {
    ...
    async fn handle_request(
        ...
        Ok(format!(
            "Request {} processed by {} (load: {:?})",
            request_id, self.name, self.cpu_load
        ))
    }
}
...
async fn chaos_08_cpu_saturation() -> ChaosResult<()> {
    ...
    let service = Arc::new(tokio::sync::RwLock::new(MockCpuIntensiveService::new(
        "cpu-service",
    )));
    ...
    }
    {
        assert_eq!(
            ...
            "All high priority should complete"
        );
        assert!(
            low_priority_completed >= 7,
            "Most low priority should complete (no starvation)"
        );
