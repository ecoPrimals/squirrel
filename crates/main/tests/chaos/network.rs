    ...
    fn handle_request(&mut self, request_id: usize) -> ChaosResult<String> {
        ...
        if should_fail(self.failure_rate) {
            ...
            Ok(format!(
                ...
                request_id, self.name, self.request_count
            ))
        }
    }
}
...
async fn chaos_05_intermittent_network_failures() -> ChaosResult<()> {
    ...
    let service = Arc::new(tokio::sync::RwLock::new(MockFlakeyService::new(
        "flakey-service",
        0.3,
    )));
    ...
    }
    {
        ...
        println!(
            "✅ Phase 1: {} requests succeeded without retries",
            m.successful
        );
    }
    ...
    }
    {
        ...
        assert!(
            m.transient_failures > 0,
            "Should have encountered transient failures"
        );
    ...
    assert!(
        severe_successes >= 15,
        "At least 75% should succeed with retries"
    );
...
async fn chaos_06_dns_resolution_failures() -> ChaosResult<()> {
    ...
    }
    {
        ...
        assert_eq!(m.dns_queries, 3);
        println!(
            "✅ Phase 2: {} cache hits (DNS cache working)",
            m.cache_hits
        );
    }
    ...
    for hostname in &["service-a.local", "service-b.local", "service-c.local"] {
        ...
        assert!(
            result.is_ok(),
            "Cached entries should work despite DNS failure"
        );
    }
    {
        let m = metrics.read().await;
    ...
    assert!(
        result.is_err(),
        "New lookups should fail when DNS is broken"
    );
    {
        let m = metrics.read().await;
    ...
    }
    {
        ...
        assert!(
            m.dns_queries > 3,
            "Should have made new DNS queries after expiration"
        );
