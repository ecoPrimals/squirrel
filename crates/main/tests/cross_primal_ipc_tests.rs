...
async fn test_capability_list_ecosystem_format() {
    ...
    assert!(result.get("capabilities").unwrap().is_array());
    let caps = result["capabilities"].as_array().unwrap();
    assert!(!caps.is_empty());

    // Domain introspection fields
    assert!(result.get("domains").unwrap().is_array());
    assert!(result.get("locality").unwrap().is_object());
    assert!(result["locality"].get("local").unwrap().is_array());
    assert!(result["locality"].get("external").unwrap().is_array());

    // Legacy "methods" object still present for backward compat
    assert!(result.get("methods").unwrap().is_object());
