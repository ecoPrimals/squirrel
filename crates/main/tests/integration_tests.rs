...
use squirrel::ecosystem::{
    EcosystemConfig, EcosystemManager, EcosystemPrimalType, EcosystemServiceRegistration,
};
...
async fn test_complete_ai_inference_workflow() {
    ...
    for (i, test_case) in test_cases.into_iter().enumerate() {
        let response = provider
            .handle_ai_inference_request(test_case)
            .await
...
async fn test_ai_inference_error_handling() {
    ...
    for (i, error_case) in error_cases.into_iter().enumerate() {
        let response = provider
            .handle_ai_inference_request(error_case)
            .await
            .expect(&format!(
                "Error test case {} should still return a response",
                i
            ));
...
async fn test_context_analysis_comprehensive() {
    ...
    for (test_input, expected_indicator) in test_cases {
        let response = provider
            .handle_context_analysis(test_input)
            .await
        ...
        let result_str = serde_json::to_string(&response["result"])
            ...
            .to_lowercase();
        assert!(
            result_str.contains(expected_indicator),
            "Analysis result should contain '{}', got: {}",
            expected_indicator,
            result_str
        );
    }
...
async fn test_session_lifecycle_management() {
    ...
    let update_response = provider
        .handle_session_management(update_request)
        .await
    ...
    let list_response = provider
        .handle_session_management(list_request)
        .await
    ...
    let end_response = provider
        .handle_session_management(end_request)
        .await
    ...
    let cleanup_response = provider
        .handle_session_management(cleanup_request)
        .await
...
async fn test_session_error_scenarios() {
    ...
    let response = provider
        .handle_session_management(invalid_get)
        .await
...
async fn test_concurrent_operations() {
    ...
    let provider =
        Arc::new(SquirrelPrimalProvider::new(config, context).expect("Failed to create provider"));
...
async fn test_provider_initialization_and_cleanup() {
    ...
    let response = provider
        .handle_ai_inference_request(test_request)
        .await
...
async fn test_memory_usage_under_load() {
    ...
    let clones_avoided = final_metrics
    ...
    assert!(
        efficiency > 0.8,
        "Efficiency should remain high under load: {}",
        efficiency
    );

    println!(
        "Load test results: {} ops, {} clones avoided, {:.2}% efficiency",
        total_ops,
        clones_avoided,
        efficiency * 100.0
    );
...
async fn test_cross_component_integration() {
    ...
    let provider =
        SquirrelPrimalProvider::new(config.clone(), context).expect("Failed to create provider");
    ...
    let ai_response = provider
        .handle_ai_inference_request(ai_request)
        .await
    ...
    let analysis_response = provider
        .handle_context_analysis(analysis_request)
        .await
        .expect("Context analysis failed");
    ...
    let session_response = provider
        .handle_session_management(session_request)
        .await
