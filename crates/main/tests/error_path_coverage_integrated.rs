...
mod error_path_tests {
    use squirrel::capability_registry::{
    ...
    async fn test_capability_discovery_empty_registry() {
        ...
        assert!(primals.is_empty(), "Empty registry should have no primals");
    }
    ...
    async fn test_capability_discovery_invalid_endpoint() {
        ...
        // Register with invalid endpoint (using ArcStr for zero-copy)
        let result = registry
            .register_primal(
                "test-invalid".into(),
                "Test Invalid".into(),
        ...
        // Registration should succeed (validation happens during health checks)
        assert!(
            result.is_ok(),
            "Registration should succeed even with invalid endpoint"
        );
        ...
        let primals = registry
            ...
            .await
            .unwrap();
        assert_eq!(primals.len(), 1, "Should have registered 1 primal");
    }
    ...
    async fn test_capability_discovery_duplicate_registration() {
        ...
        let id: ArcStr = "duplicate-test".into();
        let name: ArcStr = "Duplicate Test".into();
        let endpoint: ArcStr = "http://localhost:9999".into();
        let health: ArcStr = format!("{}/health", endpoint.as_str()).into();
...
mod migration_helper_tests {
    ...
    #[tokio::test]
    async fn test_migration_helper_with_env_vars() {
        // Set environment variables
        std::env::set_var("BEARDOG_ENDPOINT", "http://beardog-test.local:8080");
        std::env::set_var("SONGBIRD_ENDPOINT", "http://songbird-test.local:8081");

        let helper = CapabilityMigrationHelper::new().await.unwrap();

        // Manually register since auto-discovery from env happens in new()
        // but the services may not actually be running
