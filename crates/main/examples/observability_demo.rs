...
use squirrel::observability::{OperationContext, CorrelationId, utils};
...
async fn demo_basic_operation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ctx.metrics.record_phase("initialization", Duration::from_millis(50));
    
    sleep(Duration::from_millis(200)).await;
    ctx.metrics.record_phase("processing", Duration::from_millis(200));
...
async fn demo_service_call_with_correlation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut ctx = OperationContext::with_correlation_id("service_call_demo", correlation_id.clone())
...
async fn demo_api_operation_with_metadata() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ctx.metrics.record_phase("validation", Duration::from_millis(30));
    
    sleep(Duration::from_millis(80)).await;
    ctx.metrics.record_phase("database_write", Duration::from_millis(80));
    
    sleep(Duration::from_millis(20)).await;
    ctx.metrics.record_phase("response_serialization", Duration::from_millis(20));
