...
mod observability_error_tests {
    ...
    fn test_operation_context_add_metadata() {
        ...
        assert_eq!(ctx.metadata.get("service_name"), Some(&"squirrel".to_string()));
        assert_eq!(
    ...
    fn test_operation_context_record_phase() {
        ...
        ctx.metrics.record_phase("validation", Duration::from_millis(50));
        ctx.metrics.record_phase("execution", Duration::from_millis(200));
    ...
    fn test_operation_context_complete_success() {
        ...
        std::thread::sleep(Duration::from_millis(10)); // Ensure some time passes
        
        // Manually mark success instead of consuming
    ...
    fn test_operation_context_complete_failure() {
        ...
        // Manually mark failure instead of consuming
        let elapsed = ctx.start_time.elapsed();
        ctx.metrics.mark_failure(elapsed, "Service unavailable");
    ...
    fn test_operation_context_duration() {
        ...
        assert!(elapsed < Duration::from_secs(1)); // But reasonable
    }
    ...
    fn test_operation_context_full_lifecycle() {
        ...
        ctx.metrics.record_phase("process", Duration::from_millis(20));
    ...
    fn test_operation_context_retry_lifecycle() {
        ...
        // First attempt
        ctx.metrics.increment_attempts();
        ctx.metrics.record_phase("attempt_1", Duration::from_millis(50));
        
        // Second attempt
        ctx.metrics.increment_attempts();
        ctx.metrics.record_phase("attempt_2", Duration::from_millis(60));
        
        // Third attempt succeeds
        ctx.metrics.increment_attempts();
        ctx.metrics.record_phase("attempt_3", Duration::from_millis(55));
        let elapsed = ctx.start_time.elapsed();
        ctx.metrics.mark_success(elapsed);
