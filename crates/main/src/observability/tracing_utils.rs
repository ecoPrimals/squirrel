...
impl UniversalTracingCoordinator {
    ...
    async fn query_universal_adapter_for_tracing(
        &self,
    ) -> Result<Vec<TracingEndpoint>, PrimalError> {
        ...
        info!(
            "Universal adapter discovered {} tracing-capable primals",
            discovered.len()
        );
        Ok(discovered)
    }
    ...
    pub async fn start_traced_operation(
        &self,
        operation_name: String,
        attributes: HashMap<String, String>,
    ) -> Result<Uuid, PrimalError> {
        ...
        self.propagate_trace_context(operation_id, &operation_name)
            .await?;
    ...
    async fn propagate_trace_context(
        ...
        for endpoint in endpoints.iter() {
            if let Err(e) = self
                .send_trace_context(endpoint, operation_id, operation_name)
                .await
            {
                debug!(
                    "Failed to propagate trace context to {}: {}",
    ...
    async fn send_trace_context(
        ...
        if !response.status().is_success() {
            return Err(PrimalError::NetworkError(format!(
                "Trace endpoint returned error: {}",
                response.status()
            )));
        }

        debug!(
            "Successfully propagated trace context to {}",
    ...
    pub async fn finish_traced_operation(
        ...
        if let Some((_, mut operation)) = self.active_spans.remove(&operation_id) {
            ...
            info!(
                "Finished traced operation: {} (duration: {}ms)",
    ...
    async fn collect_trace_data(&self, operation_id: Uuid) -> Result<(), PrimalError> {
        ...
        for endpoint in endpoints.iter() {
            if let Ok(trace_data) = self.collect_from_endpoint(endpoint, operation_id).await {
                debug!(
                    "Collected trace data from {}: {} events",
                    endpoint.primal_type,
                    trace_data.len()
                );
    ...
    F: std::future::Future<Output = Result<T, PrimalError>>,
{
    ...
    async move {
        ...
        match &result {
            Ok(_) => info!(
                "Traced operation completed successfully: {}",
                operation_name
            ),
