...
use crate::universal::{
    ServiceCapability, ServiceEndpoint, UniversalServiceRegistration,
    UniversalSecurityProvider, UniversalSecuritySession
};
...
impl UniversalSecurityAdapter {
    ...
    pub async fn discover_security_providers_via_songbird(&self) -> Result<Vec<DiscoveredSecurityProvider>, PrimalError> {
        ...
        let security_services = self.songbird_registry
        ...
        for service_info in security_services {
            ...
            if capability_score > 0.0 {
                let provider = DiscoveredSecurityProvider {
                    ...
                    capability_score,
                    security_level: self.determine_security_level_from_songbird(&service_info).await,
                    auth_methods: self.extract_auth_methods_from_songbird(&service_info).await,
                    auth_features: self.extract_auth_features_from_songbird(&service_info).await,
        ...
        discovered_providers.sort_by(|a, b| {
            b.capability_score.partial_cmp(&a.capability_score)
            ...
            if self.config.enable_fallback {
                ...
                *self.active_provider.write().await = None; // Use Squirrel fallback
                ...
                return Err(PrimalError::InitializationFailed(
                    "No suitable security providers found via Songbird and fallback disabled".to_string()
                ));
            }
        }
    ...
    async fn calculate_mcp_capability_score(&self, service_info: &ServiceInfo) -> f64 {
        ...
        for requirement in &self.required_capabilities {
            ...
            let capability_match = service_info.capabilities.iter()
    ...
    fn matches_mcp_capability(
        ...
        match (songbird_cap, requirement.capability_type.as_str()) {
            (SongbirdCapability::Security { level, features }, "enterprise_security") => {
                level == "enterprise" && 
                requirement.required_features.iter()
                    .all(|feature| features.contains(feature))
            }
            (SongbirdCapability::Authentication { methods }, "mcp_authentication") => {
                requirement.required_features.iter()
    ...
    async fn notify_songbird_of_selection(&self, provider: &DiscoveredSecurityProvider) -> Result<(), PrimalError> {
    ...
    async fn check_songbird_provider_health(&self) -> Result<(), PrimalError> {
        ...
        if let Some(ref active) = *provider {
            ...
            let health = self.songbird_registry
                .get_service_health(&active.service_info.id)
                .await
                .map_err(|e| PrimalError::ServiceError(format!("Songbird health check error: {}", e)))?;
    ...
    async fn determine_security_level_from_songbird(&self, service_info: &ServiceInfo) -> SecurityLevel {
        // Extract security level from Songbird service metadata
        service_info.metadata.get("security_level")
            .and_then(|v| v.as_str())
    ...
    async fn extract_auth_methods_from_songbird(&self, service_info: &ServiceInfo) -> Vec<String> {
        ...
        service_info.capabilities.iter()
    ...
    async fn get_provider_health_from_songbird(&self, service_info: &ServiceInfo) -> ProviderHealthStatus {
        // Get health status from Songbird service registry
        match self.songbird_registry.get_service_health(&service_info.id).await {
            Ok(health) => {
                if health.is_healthy {
...
impl SecurityProvider for SquirrelMcpSecurityProvider {
    async fn authenticate(&self, credentials: Value) -> Result<UniversalSecuritySession, PrimalError> {
        ...
        if let Some(ai_capabilities) = credentials.get("ai_capabilities") {
            metadata.insert("client_ai_capabilities".to_string(), ai_capabilities.to_string());
        }
    ...
    async fn authorize(&self, session_id: &str, resource: &str, action: &str) -> Result<bool, PrimalError> {
        debug!("🐿️ Squirrel MCP authorization: session={}, resource={}, action={}", session_id, resource, action);
...
impl UniversalSecurityProvider for UniversalSecurityAdapter {
    ...
    async fn authenticate(&self, credentials: Value) -> Result<Self::Session, Self::Error> {
        ...
        if let Some(ref discovered_provider) = *provider {
            info!(
                ...
                discovered_provider.capability_score
            );
            
            // Create dynamic security client based on discovered provider endpoints
            let auth_endpoint = discovered_provider.service_info.endpoints.iter()
                .find(|ep| ep.name == "authenticate")
                .ok_or_else(|| PrimalError::ServiceError(
            ...
            let session = self.create_session_for_provider(&discovered_provider, credentials).await?;
    ...
    async fn authorize(&self, session_id: &str, resource: &str, action: &str) -> Result<bool, Self::Error> {
        debug!("🔐 Universal authorization check: session={}, resource={}, action={}", session_id, resource, action);
        ...
        if let Some(ref discovered_provider) = *provider {
            ...
            let authorized = self.authorize_with_provider(&discovered_provider, session_id, resource, action).await?;
            ...
            let authorized = self.fallback_provider.authorize(session_id, resource, action).await?;
    ...
    async fn revoke_session(&self, session_id: &str) -> Result<(), Self::Error> {
        ...
        if sessions.remove(session_id).is_some() {
            info!("🗑️ Session {} revoked via universal security adapter", session_id);
        } else {
            warn!("⚠️ Attempted to revoke non-existent session: {}", session_id);
        }
        Ok(())
    }
    ...
    async fn get_capabilities(&self) -> Result<Vec<ServiceCapability>, Self::Error> {
        ...
        if let Some(ref discovered_provider) = *provider {
            Ok(discovered_provider.service_info.capabilities.iter()
                .map(|cap| match cap {
                    SongbirdCapability::Authentication { methods } => ServiceCapability::Authentication { methods: methods.clone() },
                    SongbirdCapability::Authorization { features } => ServiceCapability::Authorization { features: features.clone() },
                    SongbirdCapability::Security { level, features } => {
                        if features.contains("cryptographic_proofs") {
                            ServiceCapability::Security { level: SecurityLevel::Enterprise, compliance: None }
                        } else if features.contains("cross_node_trust") {
                            ServiceCapability::Security { level: SecurityLevel::Enhanced, compliance: None }
                        } else {
                            ServiceCapability::Security { level: SecurityLevel::Standard, compliance: None }
                        }
                    },
                    SongbirdCapability::Custom { name, metadata } => ServiceCapability::Custom { capability_type: name.clone(), metadata: metadata.clone() },
                })
                .collect())
        } else {
...
impl UniversalSecurityAdapter {
    ...
    async fn create_session_for_provider(
        ...
        metadata.insert("provider_id".to_string(), provider.service_info.id.to_string());
        metadata.insert("security_level".to_string(), format!("{:?}", provider.security_level));
        metadata.insert("capability_score".to_string(), provider.capability_score.to_string());
        ...
        Ok(UniversalSecuritySession {
            ...
            user_id: Some(user_id.to_string()),
            created_at: Utc::now(),
            expires_at: Utc::now() + chrono::Duration::minutes(self.config.max_session_duration as i64),
            metadata,
        })
    }
    ...
    async fn authorize_with_provider(
        ...
        let has_auth_capability = provider.service_info.capabilities.iter()
            .any(|cap| matches!(cap, SongbirdCapability::Authorization { .. }));
            
        if !has_auth_capability {
            warn!("Provider {} lacks authorization capability", provider.service_info.name);
            return Ok(false);
        }
