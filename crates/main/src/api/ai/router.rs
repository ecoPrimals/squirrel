...
#[cfg(test)]
mod tests {
    use super::super::adapters::{AiProviderAdapter, QualityTier};
    use super::super::constraints::RoutingConstraint;
    ...
    async fn generate_text_errors_when_only_image_adapter_present() {
        let router = AiRouter::from_adapters_for_test(vec![Arc::new(MockImageOnlyAdapter {
            id: "img-only",
        })]);
        let req = TextGenerationRequest {
            prompt: "x".to_string(),
