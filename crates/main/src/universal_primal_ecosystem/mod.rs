...
impl UniversalPrimalEcosystem {
    ...
    async fn send_capability_request(
        ...
        let max_retries = 3;
        let per_attempt_timeout = timeouts::DEFAULT_REQUEST_TIMEOUT;
        let base_delay = timeouts::DEFAULT_RETRY_DELAY;
        ...
        for attempt in 1..=max_retries {
            ...
            let request_result = SafeOps::safe_with_timeout(
                per_attempt_timeout,
                || async { client.post(&request_url).json(&request).send().await },
                &format!(
                    "capability_request_attempt_{}_{}",
                    service.service_id, attempt
                ),
            )
            .await;
            ...
            if attempt < max_retries {
                ...
                // Modern async backoff: Use timeout with pending future instead of sleep
                // This is more cancellation-friendly and integrates better with async runtime
                // Equivalent to sleep but respects task cancellation
                let _ = tokio::time::timeout(delay, std::future::pending::<()>()).await;
            }
        }

        let total_duration = operation_start.elapsed();
        let final_error =
            last_error.unwrap_or_else(|| "All capability request attempts failed".to_string());
