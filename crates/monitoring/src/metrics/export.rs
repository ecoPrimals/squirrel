...
impl DefaultMetricExporter {
    ...
    pub async fn export_metrics(&self, metrics: Vec<Metric>) -> Result<()> {
        ...
        let current_len = current_metrics.len();
        if current_len > self.config.batch_size {
            let overflow = current_len - self.config.batch_size;
            // Instead of using drain, we'll use the helper function to create a new Vec with only the newest metrics
            let new_metrics = current_metrics.iter().skip(overflow).cloned().collect::<Vec<_>>();
    ...
    pub async fn get_metrics(&self) -> Result<Vec<Metric>> {
        ...
        let mut cloned_metrics = Vec::new();
        for metric in metrics.iter() {
            cloned_metrics.push(metric.clone());
        }
        Ok(cloned_metrics)
    }
    ...
    pub async fn handle_batch(&mut self, metrics: Vec<Metric>) -> Result<()> {
        let mut lock = self.metrics.write().await;
        
        // Add new metrics
        crate::metrics::add_to_write_guard(&mut lock, &metrics);
        
        // Check if we need to cap the size
        crate::metrics::retain_newest_metrics(&mut lock, self.config.batch_size);
