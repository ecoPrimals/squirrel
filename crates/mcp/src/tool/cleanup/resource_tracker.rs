...
impl ResourceTracker {
    ...
    pub async fn track_memory_allocation(&self, bytes: u64) -> Result<(), String> {
        ...
        debug!("Tool {} allocated {}B (now at {}B)", 
               self.data.tool_id, bytes, current + bytes);
        Ok(())
    }
    ...
    pub async fn track_memory_deallocation(&self, bytes: u64) -> Result<(), String> {
        ...
        debug!("Tool {} deallocated {}B (now at {}B)", 
               self.data.tool_id, bytes, current - bytes);
        Ok(())
    }
    ...
    pub async fn track_cpu_time(&self, milliseconds: u64) -> Result<(), String> {
        let current = self.data.cpu_time_ms.fetch_add(milliseconds, Ordering::SeqCst);
        debug!("Tool {} used {}ms CPU time (now at {}ms)", 
               self.data.tool_id, milliseconds, current + milliseconds);
        Ok(())
    }
    ...
    pub async fn track_file_handle_open(&self, path: &str) -> Result<(), String> {
        ...
        debug!("Tool {} opened file {} (now has {} open files)", 
               self.data.tool_id, path, handles.len());
        Ok(())
    }
    ...
    pub async fn track_file_handle_close(&self, path: &str) -> Result<(), String> {
        ...
        if handles.remove(path) {
            debug!("Tool {} closed file {} (now has {} open files)", 
                   self.data.tool_id, path, handles.len());
            Ok(())
        } else {
            Err(format!("File handle {} not tracked for tool {}", 
    ...
    pub async fn track_network_connection_open(&self, endpoint: &str) -> Result<(), String> {
        ...
        debug!("Tool {} opened connection to {} (now has {} open connections)", 
               self.data.tool_id, endpoint, connections.len());
        Ok(())
    }
    ...
    pub async fn track_network_connection_close(&self, endpoint: &str) -> Result<(), String> {
        ...
        if connections.remove(endpoint) {
            debug!("Tool {} closed connection to {} (now has {} open connections)", 
                   self.data.tool_id, endpoint, connections.len());
            Ok(())
        } else {
            Err(format!("Network connection {} not tracked for tool {}", 
    ...
    pub async fn cleanup(&self) -> Result<(), String> {
        ...
        if !usage.file_handles.is_empty() {
            warn!("Tool {} has {} unclosed file handles during cleanup", 
                  self.data.tool_id, usage.file_handles.len());
            // Here we would close files
        }
        
        if !usage.network_connections.is_empty() {
            warn!("Tool {} has {} unclosed network connections during cleanup",
                  self.data.tool_id, usage.network_connections.len());
