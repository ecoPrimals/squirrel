...
pub use cleanup_hook::{CleanupHook, BasicCleanupHook};
pub use resource_manager::{ResourceManager, ResourceUsage, ResourceLimits};
...
mod tests {
    ...
    async fn test_basic_resource_manager() {
        ...
        rm.initialize_tool(
            ...
            ResourceLimits::default()
        ).await.expect("Failed to initialize tool");
        
        // Check initial usage
        let usage = rm.get_usage("test-tool").await.expect("Failed to get usage");
        ...
        let within_limits = rm.check_limits("test-tool").await.expect("Failed to check limits");
        assert!(within_limits);
        
        // Test cleanup
        rm.cleanup_tool("test-tool").await.expect("Failed to cleanup tool");
    ...
    async fn test_basic_cleanup_hook() {
        ...
        // Register the tool
        cleanup_hook.register_tool(&tool).await.expect("Failed to register tool");
        
        // Test the reset operation
        cleanup_hook.reset_tool("test-tool").await.expect("Failed to reset tool");
        
        // Test cleanup
        cleanup_hook.cleanup_tool("test-tool").await.expect("Failed to cleanup tool");
