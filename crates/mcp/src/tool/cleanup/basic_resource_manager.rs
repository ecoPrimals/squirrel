...
use super::resource_manager::{ResourceManager, ResourceLimits, ResourceUsage};
...
impl ResourceManager for BasicResourceManager {
                ...
                if new_limits.max_memory_bytes > max.max_memory_bytes ||
                   ...
                   new_limits.max_network_connections > max.max_network_connections {
                    return Err(ToolError::ResourceError(
                        format!("Requested limits exceed maximum allowed limits for tool {}", tool_id)
                    ));
                }
            } else {
    ...
    async fn check_limits(&self, tool_id: &str) -> Result<bool, ToolError> {
        ...
        let within_limits = 
            usage.memory_bytes <= tool_limits.max_memory_bytes &&
