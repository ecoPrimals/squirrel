...
impl TryFrom<EncryptionInfoHelper> for EncryptionInfo { 
    ...
    fn try_from(helper: EncryptionInfoHelper) -> std::result::Result<Self, Self::Error> {
        ...
        Ok(EncryptionInfo { 
            format: helper.format.try_into()?, // Propagates ProtocolError
...
impl TryFrom<SecurityMetadataHelper> for SecurityMetadata {
    ...
    fn try_from(helper: SecurityMetadataHelper) -> std::result::Result<Self, Self::Error> {
        let security_level = match helper.security_level.as_deref() {
            ...
            Some(unknown) => {
                warn!("Unknown security level string '{}', defaulting to Standard.", unknown);
                SecurityLevel::Standard
            },
            None => SecurityLevel::Standard,
        };

        let encryption_info: Option<EncryptionInfo> = helper
            .encryption_info
