...
use crate::types::{EncryptionFormat, SecurityLevel};
use crate::security::rbac::{Permission, PermissionScope, RBACManager, Role, Action};
...
// Re-export types
pub use encryption::{Encryption, EncryptionManager};
...
impl SecurityManagerImpl {
    ...
    pub fn new(config: SecurityConfig) -> Result<Arc<Self>> {
        ...
        for role in &config.default_roles {
            ...
            if let Err(e) = role_result {
                return Err(match e {
                    MCPError::Security(security_err) => MCPError::Security(security_err),
    ...
    pub async fn authenticate(&self, credentials: &Credentials) -> Result<String> {
        ...
        if !Self::verify_credentials(credentials) {
            ...
            return Err(MCPError::Security(SecurityError::InvalidCredentials(
    ...
    pub async fn authorize(
        ...
        let session = match session {
            Some(s) => s,
            None => {
                return Err(MCPError::Security(SecurityError::InvalidToken(
    ...
    pub async fn encrypt(&self, session_id: &str, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let unbound_key = UnboundKey::new(&CHACHA20_POLY1305, &key.key).map_err(|_| {
            MCPError::Security(SecurityError::EncryptionFailed("Invalid key".into()))
        })?;
        ...
        let tag = sealing_key
            ...
            .map_err(|_| {
                MCPError::Security(SecurityError::EncryptionFailed("Encryption failed".into()))
            })?;

        let mut result = Vec::with_capacity(NONCE_LEN + in_out.len() + CHACHA20_POLY1305.tag_len());
    ...
    pub async fn decrypt(&self, session_id: &str, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let unbound_key = UnboundKey::new(&CHACHA20_POLY1305, &key.key).map_err(|_| {
            MCPError::Security(SecurityError::DecryptionFailed("Invalid key".into()))
        })?;

        let nonce_gen = NonceGen::new(nonce.try_into().map_err(|_| {
            MCPError::Security(SecurityError::DecryptionFailed("Invalid nonce".into()))
        })?);
        ...
        opening_key
            ...
            .map_err(|_| {
                MCPError::Security(SecurityError::DecryptionFailed("Decryption failed".into()))
            })?;

        Ok(in_out[..in_out.len() - CHACHA20_POLY1305.tag_len()].to_vec())
    }
    ...
    async fn check_auth_attempts(&self, credentials: &Credentials) -> Result<()> {
        ...
        if let Some(attempt) = state.auth_attempts.get(&credentials.client_id) {
            if attempt.count >= self.config.max_auth_attempts {
                return Err(
                    MCPError::Security(SecurityError::AuthenticationFailed(format!(
                        "Too many failed attempts for client {}",
    ...
    async fn generate_session_key(&self, session_id: &str) -> Result<()> {
        ...
        let session_key = SessionKey {
            key,
            created_at: Utc::now(),
            expires_at: Utc::now() + Duration::seconds(self.config.token_validity),
        };

        // Store the session key
        self.key_manager.session_keys.write().await.insert(session_id.to_string(), session_key);
    ...
    async fn get_session_key(&self, session_id: &str) -> Result<SessionKey> {
        ...
        match keys.get(session_id).cloned() {
            Some(key) => Ok(key),
            None => Err(MCPError::Security(SecurityError::InvalidToken(
    ...
    pub async fn cleanup_expired_sessions(&self) -> Result<()> {
...

        {
            ...
            state.active_sessions.retain(|session| {
                let is_valid = session.expires_at > now;
                if !is_valid {
                    expired_session_ids.push(session.id.clone());
                }
                is_valid
            });
        }

        // Clean up session keys for expired sessions
    ...
    pub async fn assign_role(&self, user_id: String, role_id: String) -> Result<()> {
        ...
        match rbac_manager.assign_role(user_id, role_id) {
            Ok(_) => Ok(()),
            Err(e) => Err(MCPError::Security(SecurityError::InvalidRole(
    ...
    pub async fn assign_role_by_name(&self, user_id: String, role_name: String) -> Result<()> {
        let role_id = {
            let rbac_manager = self.rbac_manager.read().await;
            match rbac_manager.get_role_by_name(&role_name) {
    ...
    pub async fn create_role(
        ...
        let permissions_set = permissions
            .into_iter()
            .map(|p| {
                // Parse the permission string format: "resource:action"
                let parts: Vec<&str> = p.split(':').collect();
                if parts.len() != 2 {
                    return Err(MCPError::Security(SecurityError::InvalidPermissionFormat(
                        format!("Invalid permission format: {p}")
                    )));
                }

                let _resource = parts[0].to_string();
                let action = match parts[1].to_lowercase().as_str() {
                    ...
                    "admin" => Action::Admin,
                    _ => {
                        return Err(MCPError::Security(SecurityError::InvalidActionInPermission(
                            format!("Invalid action in permission: {p}")
                        )))
                    }
                };

                Ok(Permission {
                    id: format!("{_resource}:{}", action),
                    name: p.clone(),
                    resource: _resource,
                    action,
                    resource_id: None,
        ...
        match rbac_manager.create_role(name, description, permissions_set, parent_roles_set) {
            Ok(role) => Ok(role),
            Err(e) => Err(MCPError::Security(SecurityError::ErrorCreatingRole(
    ...
    pub async fn create_role_with_id(
        ...
        match rbac_manager.create_role_with_id(
            id,
            name,
            description,
            permissions_set,
...
impl SecurityManager for SecurityManagerImpl {
    ...
    async fn authorize(
        ...
        self.authorize(token, required_security_level, required_permission)
            .await
    }
    ...
    async fn create_role(
        ...
        let permission_strings: Vec<String> = permissions.iter().map(|p| p.id.clone()).collect();

        // Convert HashSet to Vec for parent roles
        let parent_roles_vec: Vec<String> = parent_roles.into_iter().collect();

        self.create_role(name, description, permission_strings, parent_roles_vec)
            .await
    }
    ...
    async fn create_role_with_id(
        ...
        self.create_role_with_id(id, name, description, permission_strings, parent_roles_vec)
            .await
    }
...
mod tests {
    ...
    async fn test_authorization() {
        ...
        let session = security
            .authorize(&token, SecurityLevel::Standard, None)
            .await;
    ...
    async fn test_rbac_integration() {
        ...
        assert!(
            security
                .has_permission(&credentials.client_id, &read_perm)
                .await
        );
        assert!(
            security
                .has_permission(&credentials.client_id, &write_perm)
                .await
        );
    }
}
