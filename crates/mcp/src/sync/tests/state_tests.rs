...
async fn test_state_operation_recording() {
    ...
    assert_eq!(
        change.context_id, context.id,
        "Change should have correct context ID"
    );
    assert_eq!(
        change.operation,
    ...
    state_manager
        .apply_change(update_change.clone())
        .await
        .expect("Failed to apply change");

    // ASSERT: Verify updated change
    assert_eq!(
        update_change.context_id, context.id,
        "Change should have correct context ID"
    );
    assert_eq!(
...
async fn test_state_change_subscription() {
    ...
    let received1 = rx1
        .recv()
        .await
        .expect("Subscriber 1 failed to receive change");
    let received2 = rx2
        .recv()
        .await
        .expect("Subscriber 2 failed to receive change");

    assert_eq!(
        received1.id, change.id,
        "Subscriber 1 should receive exact change"
    );
    assert_eq!(
        received2.id, change.id,
        "Subscriber 2 should receive exact change"
    );
}
