...
use crate::sync::state::{StateChange, StateOperation as InternalStateOperation}; // Keep these
use crate::sync::state::StateSyncManager; // Import manager separately
use crate::MCPError;
    ...
    pub async fn synchronize(&self) -> Result<SyncResult> {
        ...
        match &result {
            Ok(sync_result) => {
                ...
                state.sync_count += 1;
                state.last_version = Some(sync_result.version);
    ...
    async fn sync_internal(&self) -> Result<SyncResult> {
        ...
        Ok(SyncResult {
            ...
            duration_ms,
                                ...
                                version: response.get_ref().current_server_version,
                            })
                        }
                        Err(e) => {
                            // Handle tonic error
...
pub(crate) use crate::sync::state::StateOperation; 