...
async fn test_retry_different_backoff_strategies() {
    ...
    // Should have same delay each time
    assert_eq!(retry1.calculate_delay(1), retry1.calculate_delay(2));
    ...
    assert!(delay3 > delay1 * 2);
    
    // Test Linear backoff
    let retry3 = RetryMechanism::new(RetryConfig {
        base_delay: Duration::from_millis(10),
    ...
    let delay1 = retry3.calculate_delay(1);
    let delay2 = retry3.calculate_delay(2);
    let delay3 = retry3.calculate_delay(3);
    
    assert!(delay2 > delay1);
    assert!(delay3 > delay2);
    assert_eq!(delay2 - delay1, delay3 - delay2);
}
