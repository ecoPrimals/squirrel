...
use squirrel_mcp::tool::{
    lifecycle::SecurityLifecycleHook, BasicLifecycleHook, BasicToolExecutor, Capability,
    CompositeLifecycleHook, ExecutionStatus, Parameter, ParameterType, RemoteToolExecutor, Tool,
    ToolError, ToolManager, ToolState,
};
        ...
        if let Some(a) = context.parameters.get("a").and_then(|v| v.as_f64()) {
            if let Some(b) = context.parameters.get("b").and_then(|v| v.as_f64()) {
                ...
                Err(ToolError::ValidationFailed(
                    "Parameter 'b' must be a number".to_string(),
                ))
            }
        } else {
            Err(ToolError::ValidationFailed(
                "Parameter 'a' must be a number".to_string(),
            ))
        }
    });
...
fn create_text_executor() -> BasicToolExecutor {
    ...
    executor.register_handler("reverse", |context| {
        if let Some(text) = context.parameters.get("text").and_then(|v| v.as_str()) {
            ...
            Err(ToolError::ValidationFailed(
                "Parameter 'text' must be a string".to_string(),
            ))
        }
    });
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager
        .register_tool(calculator_tool, calculator_executor)
        .await?;
    ...
    for tool in &tools {
        let state = all_states
            .get(&tool.id)
            .copied()
            .unwrap_or(ToolState::Unregistered);
    ...
    // Echo
    {
        ...
        assert_eq!(result.status, ExecutionStatus::Success);
        assert_eq!(
            result.output.as_ref().unwrap()["message"],
            json!("Hello, MCP!")
        );
    }
    ...
    // Count
    {
        ...
        info!("  Result: {:?}", result.output);
        assert_eq!(result.status, ExecutionStatus::Success);
        let char_count = result.output.as_ref().unwrap()["char_count"]
            .as_i64()
            .unwrap();
        let word_count = result.output.as_ref().unwrap()["word_count"]
            .as_i64()
            .unwrap();
    ...
    // Remote echo
    {
        ...
        assert!(
            ...
            "Expected error message for remote service failure"
        );
    }
