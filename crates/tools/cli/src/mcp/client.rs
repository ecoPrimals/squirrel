...
impl MCPClient {
    ...
    ///
    /// A Result indicating success or an error
    #[allow(clippy::too_many_lines)] // REPL command loop; split later if this grows further
...
mod tests {
    ...
    fn request_and_notification_shapes_via_protocol() {
        ...
        let req =
            MCPMessage::new_request(id.clone(), "cmd".to_string(), Some(json!({"args": ["a"]})));
        ...
        let n =
            MCPMessage::new_notification("n1".to_string(), "topic".to_string(), Some(json!({})));
    ...
    fn error_response_message_maps_to_command_error() {
        let err_json =
            MCPMessage::new_error("e1".to_string(), "cmd".to_string(), "bad".to_string())
                .to_json()
                .expect("serialize");
