        ...
        let nested_condition = RuleCondition::All {
            conditions: vec![
                RuleCondition::Exists {
                    path: "data.value1".to_string(),
                },
                RuleCondition::Exists {
                    path: "data.value2".to_string(),
                },
            ],
        };
...
async fn test_rule_directory_manager() {
    ...
    // Create a category
    let category_rule_content =
        utils::create_rule_template("category-rule", "Category Rule", "test-category");
    let category_file_path = manager
        .create_rule_file(
            "category-rule",
            Some("test-category"),
