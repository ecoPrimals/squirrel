...
impl MockCommandRegistry {
    ...
    fn register(&mut self, name: &str, command: Box<dyn MockCommand + Send + Sync>) -> MockCommandResult<()> {
        self.commands.insert(name.to_string(), command);
        Ok(())
    }
    
    fn execute(&self, name: &str, args: &[String]) -> MockCommandResult<String> {
...
impl CommandRegistryAdapter {
    ...
    fn register_command(&self, command: Box<dyn MockCommand + Send + Sync>) -> Result<(), String> {
...
impl McpCommandAdapter {
    ...
    async fn execute_with_auth(&self, command: &str, args: &[String], auth: Auth) -> Result<String, String> {
        // Check authorization
        match auth {
