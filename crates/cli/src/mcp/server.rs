...
impl MCPServer {
        ...
        F: Fn(MCPMessage) -> MCPResult<MCPMessage> + Send + Sync + 'static,
    {
        ...
        {
            let mut handlers = self.command_handlers.lock().unwrap();
    ...
    pub fn start(&self) -> MCPResult<()> {
        ...
        if *running {
            return Err(MCPError::ProtocolError("Server already running".to_string()));
        }
    ...
    pub fn stop(&self) -> MCPResult<()> {
        ...
        *running = false;
        ...
        for (_id, client) in clients.drain() {
            let _ = client.lock().unwrap().shutdown(std::net::Shutdown::Both);
        }
    ...
    pub fn broadcast_notification(&self, topic: &str, payload: Option<Value>) -> MCPResult<()> {
        let notification = MCPMessage::new_notification(
        ...
        let subscribers = {
            ...
            topic_subscribers.get(topic).cloned().unwrap_or_else(HashSet::new)
        };
    ...
    ///
    /// A result indicating success or failure
    pub fn send_notification(&self, client_id: &str, topic: &str, payload: Option<Value>) -> MCPResult<()> {
        let notification = MCPMessage::new_notification(
            Uuid::new_v4().to_string(),
            topic.to_string(),
            payload
        ...
        if let Some(stream) = clients.get(client_id) {
            ...
            return Err(MCPError::ProtocolError(format!("Client {} not found", client_id)));
        }
        ...
        while *self.running.lock().unwrap() {
                match listener.accept() {
                    ...
                    // Store the client connection
                    {
                        ...
                        clients.insert(client_id.clone(), Arc::new(Mutex::new(stream.try_clone()?)));
                    }
        ...
        let topic = if let Some(payload) = &request.payload {
            if let Some(topic) = payload.get("topic") {
                if let Some(topic_str) = topic.as_str() {
                    ...
                    return Err(MCPError::ProtocolError("Topic must be a string".to_string()));
                }
            } else {
                return Err(MCPError::ProtocolError("Missing 'topic' field in subscription request".to_string()));
            }
        } else {
            return Err(MCPError::ProtocolError("Missing payload in subscription request".to_string()));
        };
    ...
    ///
    /// A result containing an optional response message
    fn handle_unsubscribe(&self, client_id: &str, request: MCPMessage) -> MCPResult<Option<String>> {
        // Extract topic from request payload
        let topic = if let Some(payload) = &request.payload {
            if let Some(topic) = payload.get("topic") {
                if let Some(topic_str) = topic.as_str() {
                ...
                return Err(MCPError::ProtocolError("Missing 'topic' field in unsubscription request".to_string()));
            }
        } else {
            return Err(MCPError::ProtocolError("Missing payload in unsubscription request".to_string()));
        };
        ...
        if let Some(registry) = &self.command_registry {
            ...
            match result {
                Ok(output) => {
                    ...
                    let response = MCPMessage::new_error(
                        message.id,
                        message.command,
                        error
                    );
            ...
            // Unknown command
            Err(MCPError::ProtocolError(format!("Unknown command: {}", command_name)))
        }
    }
    ...
    fn handle_notification(&self, client_id: String, notification: MCPMessage) -> MCPResult<()> {
        ...
        debug!("Received notification '{}' from client {}", topic, client_id);
            ...
            for subscriber_id in subscribers {
                if let Some(stream) = clients.get(&subscriber_id) {
                    ...
                    if let Err(e) = stream.write_all(format!("{}\n", json).as_bytes()) {
                        warn!("Failed to forward notification to client {}: {}", subscriber_id, e);
                    }
                }
            }
        }
