...
impl MCPClient {
    ...
    pub fn connect(&mut self, timeout: Option<Duration>) -> MCPResult<()> {
        ...
        let stream = TcpStream::connect_timeout(&address.parse().unwrap(), timeout_to_use)
            .map_err(|e| MCPError::ConnectionError(format!("Failed to connect to {}: {}", address, e)))?;
    ...
    fn start_notification_listener(&self, stream: TcpStream) -> MCPResult<()> {
                        ...
                        match MCPMessage::from_json(line.trim()) {
                            Ok(message) => {
                                ...
                                if message.message_type == MCPMessageType::Notification {
                                    ...
                                    debug!("Ignored non-notification message: {:?}", message.message_type);
                                }
                            }
                            Err(e) => {
        ...
        // Store thread handle
        let mut listener_thread = self.listener_thread.lock().unwrap();
    ...
    pub fn disconnect(&self) -> MCPResult<()> {
        ...
        if connection.is_some() {
            ...
            for topic in self.subscriptions.lock().unwrap().keys() {
    ...
    pub fn send_command(&self, command: &str, args: Option<serde_json::Value>) -> MCPResult<MCPMessage> {
        // Ensure connected
    ...
    ///
    /// A Result indicating success or an error
    pub fn send_notification(&self, topic: &str, payload: Option<serde_json::Value>) -> MCPResult<()> {
        // Ensure connected
        if !self.is_connected() {
            return Err(MCPError::ConnectionError("Not connected to MCP server".to_string()));
        }
    ...
    fn send_subscribe_message(&self, topic: &str) -> MCPResult<()> {
        self.send_command("subscribe", Some(json!({
            "topic": topic
        })))?;
        Ok(())
    }
    ...
    fn send_unsubscribe_message(&self, topic: &str) -> MCPResult<()> {
        ...
        if let Err(e) = self.send_command("unsubscribe", Some(json!({
            "topic": topic
        }))) {
            warn!("Error sending unsubscribe command for topic {}: {}", topic, e);
        }
        Ok(())
    }
    ...
    fn send_message(&self, message: &MCPMessage) -> MCPResult<MCPMessage> {
        ...
        if let Some(stream) = &*connection {
            ...
            Err(MCPError::ConnectionError("Not connected to MCP server".to_string()))
        }
    }
