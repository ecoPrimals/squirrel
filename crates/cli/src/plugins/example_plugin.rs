...
impl Command for ExampleCommand {
    ...
    fn execute(&self, _args: &[String]) -> commands::CommandResult<String> {
...
impl ExamplePlugin {
    ...
    pub fn new() -> Self {
        let commands = vec![
            Arc::new(ExampleCommand) as Arc<dyn Command>,
        ];
...
impl Plugin for ExamplePlugin {
    ...
    async fn execute(&self, args: &[String]) -> Result<String, PluginError> {
        ...
        match args[0].as_str() {
            ...
            "info" => Ok(format!("Example plugin: {} v{}\n{}", 
                self.name, 
                self.version,
                self.description.as_str()
            )),
            _ => Err(PluginError::Unknown(format!("Unknown plugin command: {}", args[0]))),
        }
    }
