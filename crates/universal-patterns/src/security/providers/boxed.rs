...
use super::{BeardogSecurityProvider, LocalSecurityProvider};

/// Runtime-selected universal security provider (Beardog primary, local fallback, etc.).
///
...
pub enum UniversalSecurityProviderBox {
    /// Beardog-backed provider.
    Beardog(Arc<BeardogSecurityProvider>),
...
impl UniversalSecurityProvider for UniversalSecurityProviderBox {
    async fn authenticate(&self, credentials: &Credentials) -> Result<AuthResult, SecurityError> {
        match self {
            Self::Beardog(p) => p.authenticate(credentials).await,
    ...
    async fn authorize(
        ...
        match self {
            Self::Beardog(p) => p.authorize(principal, action, resource).await,
    ...
    async fn encrypt(&self, data: &[u8]) -> Result<Vec<u8>, SecurityError> {
        match self {
            Self::Beardog(p) => p.encrypt(data).await,
    ...
    async fn decrypt(&self, encrypted_data: &[u8]) -> Result<Vec<u8>, SecurityError> {
        match self {
            Self::Beardog(p) => p.decrypt(encrypted_data).await,
    ...
    async fn sign(&self, data: &[u8]) -> Result<Vec<u8>, SecurityError> {
        match self {
            Self::Beardog(p) => p.sign(data).await,
    ...
    async fn verify(&self, data: &[u8], signature: &[u8]) -> Result<bool, SecurityError> {
        match self {
            Self::Beardog(p) => p.verify(data, signature).await,
    ...
    async fn audit_log(
        ...
        match self {
            Self::Beardog(p) => p.audit_log(operation, context).await,
    ...
    async fn health_check(
        ...
        match self {
            Self::Beardog(p) => UniversalSecurityProvider::health_check(p.as_ref()).await,
...
impl UniversalSecurityService for UniversalSecurityProviderBox {
    fn get_capabilities(&self) -> Vec<SecurityCapability> {
        match self {
            Self::Beardog(p) => p.get_capabilities(),
    ...
    fn get_service_info(&self) -> SecurityServiceInfo {
        match self {
            Self::Beardog(p) => p.get_service_info(),
    ...
    async fn handle_security_request(
        ...
        match self {
            Self::Beardog(p) => p.handle_security_request(request).await,
    ...
    async fn health_check(&self) -> Result<ServiceHealth, SecurityError> {
        match self {
            Self::Beardog(p) => UniversalSecurityService::health_check(p.as_ref()).await,
            ...
            Self::Beardog(p) => {
                let Some(inner) = Arc::get_mut(p) else {
                    return Err(SecurityError::configuration(
                        "Cannot initialize shared Beardog security service",
                    ));
                };
