...
async fn test_beardog_provider_get_capabilities() {
    ...
    assert_eq!(
        capabilities.len(),
        4,
        "Beardog provider should have 4 capabilities"
    );
...
async fn test_beardog_provider_initialize() {
    let config = SecurityServiceConfig::default();
    let mut provider = BeardogSecurityProvider::new(config)
        .await
        .expect("test: operation should succeed");
...
async fn test_multi_provider_registry() {
    ...
    let beardog_provider = BeardogSecurityProvider::new(beardog_config)
        .await
        .expect("test: operation should succeed");
    registry
    ...
    // Use find_optimal_service for flexible capability matching
    let requirements = vec![SecurityCapability::Authentication {
        methods: vec![AuthMethod::None],
        multi_factor: false,
        session_management: false,
    ...
    // Find compliance capability (only beardog has it) - must match exactly
