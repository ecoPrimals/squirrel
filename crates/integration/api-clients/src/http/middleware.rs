                        ...
                        match self.limiter.check() {
                            ...
                            Err(negative) => {
                                ...
                                match self.limiter.check() {
                                    ...
                                    Err(_) => {
                                        error!(
                                            "Rate limit still exceeded after waiting for {}",
                                            key
                                        );
                                        request
                                    }
...
impl Middleware for LoggingMiddleware {
    async fn process_request(&self, request: RequestBuilder) -> Result<RequestBuilder> {
        // PRODUCTION SAFE: Handle request cloning for logging with proper error handling
        let request_for_logging = match request.try_clone() {
            Some(cloned_request) => Some(cloned_request),
            None => {
                warn!("Failed to clone request for logging");
                // Try one more time
                match request.try_clone() {
                    Some(second_clone) => Some(second_clone),
                    None => {
                        // If we still can't clone, log the error but continue without logging
                        error!("CRITICAL: Failed to clone request twice for logging - continuing without request logging");
                        None
                    }
                }
            }
        ...
        // Only build the request for logging if we have a cloned request and debug or more verbose
        if let Some(request_clone) = request_for_logging {
