...
impl ContextAdapter {
        ...
        if let Some(plugin_manager) = &self.plugin_manager {
            ...
            // Store transformations in the local cache
            {
                ...
                for transformation in transformations {
                    transformations_lock
                        .insert(transformation.get_id().to_string(), transformation);
                }
            }
    ...
    pub async fn transform_data(&self, transformation_id: &str, data: Value) -> Result<Value> {
        ...
        if let Some(transformation) = transformation {
            ...
            Err(anyhow::anyhow!(
                ContextAdapterError::TransformationNotFound(transformation_id.to_string())
                    .to_string(),
            ))
        }
    }
...
impl ContextAdapter {
    ...
    pub async fn get_context(&self, id: &str) -> Result<AdapterContextData> {
        let contexts = self.contexts.read().await;
        contexts.get(id).cloned().ok_or_else(|| {
            anyhow::anyhow!(
    ...
    pub fn get_plugin_manager(
        &self,
    ) -> Option<&Arc<squirrel_context::plugins::ContextPluginManager>> {
