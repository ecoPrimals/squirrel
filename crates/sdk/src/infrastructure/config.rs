...
impl McpClientConfig {
    ...
    pub fn from_env() -> Self {
        let server_url = std::env::var(env_vars::mcp::SERVER_URL).unwrap_or_else(|_| {
            #[cfg(target_arch = "wasm32")]
            {
                let port = std::env::var(env_vars::mcp::SERVER_PORT)
                    .ok()
        ...
        Self {
            server_url,
            timeout_ms: std::env::var(env_vars::mcp::TIMEOUT_MS)
                .unwrap_or_else(|_| "30000".to_string())
                .parse()
                .unwrap_or(30000),
            max_message_size: std::env::var(env_vars::mcp::MAX_MESSAGE_SIZE)
                .unwrap_or_else(|_| "1048576".to_string())
                .parse()
                .unwrap_or(1024 * 1024),
            protocol_version: std::env::var(env_vars::mcp::PROTOCOL_VERSION)
                .unwrap_or_else(|_| "1.0".to_string()),
            max_reconnect_attempts: std::env::var(env_vars::mcp::MAX_RECONNECT_ATTEMPTS)
                .unwrap_or_else(|_| "3".to_string())
                .parse()
                .unwrap_or(3),
            reconnect_delay_ms: std::env::var(env_vars::mcp::RECONNECT_DELAY_MS)
...
impl LoggingConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            min_level: std::env::var(env_vars::logging::LEVEL)
                .unwrap_or_else(|_| "info".to_string()),
            max_entries: std::env::var(env_vars::logging::MAX_ENTRIES)
            ...
            send_to_host: std::env::var(env_vars::logging::SEND_TO_HOST)
                ...
                .parse()
                .unwrap_or(true),
            rotation_size: std::env::var(env_vars::logging::ROTATION_SIZE)
                .unwrap_or_else(|_| "1048576".to_string())
                .parse()
                .unwrap_or(1024 * 1024),
        }
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            port: std::env::var(env_vars::network::NETWORK_PORT)
                ...
                .unwrap_or_else(|| get_service_port("websocket")),
            max_connections: std::env::var(env_vars::network::NETWORK_MAX_CONNECTIONS)
                .unwrap_or_else(|_| "100".to_string())
                .parse()
                .unwrap_or(100),
            connection_timeout_ms: std::env::var(env_vars::network::NETWORK_CONNECTION_TIMEOUT_MS)
                .unwrap_or_else(|_| "30000".to_string())
                .parse()
                .unwrap_or(30000),
            read_timeout_ms: std::env::var(env_vars::network::NETWORK_READ_TIMEOUT_MS)
                .unwrap_or_else(|_| "30000".to_string())
                .parse()
                .unwrap_or(30000),
            write_timeout_ms: std::env::var(env_vars::network::NETWORK_WRITE_TIMEOUT_MS)
                .unwrap_or_else(|_| "30000".to_string())
                .parse()
                .unwrap_or(30000),
        }
...
impl HttpConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            default_timeout_ms: std::env::var(env_vars::http::DEFAULT_TIMEOUT_MS)
                .unwrap_or_else(|_| "30000".to_string())
                .parse()
                .unwrap_or(30000),
            max_request_size: std::env::var(env_vars::http::MAX_REQUEST_SIZE)
                .unwrap_or_else(|_| "10485760".to_string())
                .parse()
                .unwrap_or(10 * 1024 * 1024),
            max_response_size: std::env::var(env_vars::http::MAX_RESPONSE_SIZE)
                .unwrap_or_else(|_| "10485760".to_string())
                .parse()
                .unwrap_or(10 * 1024 * 1024),
            max_redirects: std::env::var(env_vars::http::MAX_REDIRECTS)
                .unwrap_or_else(|_| "10".to_string())
                .parse()
                .unwrap_or(10),
            user_agent: std::env::var(env_vars::http::USER_AGENT)
...
impl PerformanceConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            max_plugin_id_length: std::env::var(env_vars::performance::MAX_PLUGIN_ID_LENGTH)
                .unwrap_or_else(|_| "64".to_string())
                .parse()
                .unwrap_or(64),
            string_pool_capacity: std::env::var(env_vars::performance::STRING_POOL_CAPACITY)
                .unwrap_or_else(|_| "1000".to_string())
                .parse()
                .unwrap_or(1000),
            batch_processor_size: std::env::var(env_vars::performance::BATCH_PROCESSOR_SIZE)
                .unwrap_or_else(|_| "100".to_string())
                .parse()
                .unwrap_or(100),
            fs_buffer_size: std::env::var(env_vars::performance::FS_BUFFER_SIZE)
                .unwrap_or_else(|_| "8192".to_string())
                .parse()
                .unwrap_or(8192),
            session_timeout_seconds: std::env::var(env_vars::performance::SESSION_TIMEOUT_SECONDS)
