...
impl ContextPluginManager {
    ...
    /// Register a plugin
    pub async fn register_plugin(&self, plugin: Box<dyn ContextPlugin>) -> Result<(), ContextError> {
    ...
    pub async fn transform(&self, transformation_id: &str, data: Value) -> Result<Value, ContextError> {
        // Find the transformation
        let transformations = self.transformations.read().await;
        let transformation = transformations.iter()
            .find(|t| t.get_id() == transformation_id)
            .ok_or(ContextError::TransformationNotFound(transformation_id.to_string()))?;
        
        // Apply the transformation
        transformation.transform(data).await
            .map_err(|e| ContextError::TransformationFailed(transformation_id.to_string(), e.to_string()))
    }
    ...
    pub async fn get_adapter_metadata(&self, adapter_id: &str) -> Result<AdapterMetadata, ContextError> {
