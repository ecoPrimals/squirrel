...
impl ActionExecutor {
    ...
    pub fn with_default_context(plugin_manager: Arc<RulePluginManager>, default_context: Value) -> Self {
        Self {
            plugin_manager,
    ...
    pub async fn execute_rule_actions(&self, rule: &Rule, context: Option<&mut Value>) -> Result<Value> {
    ...
    pub async fn execute_action(&self, action: &RuleAction, context: &mut Value) -> Result<()> {
        match action {
            ...
            RuleAction::ExecuteTransformation { id, input_path, output_path, config: _config } => {
                // Get the transformation
                let transformation = self.plugin_manager.get_transformation(id).await?;
                
                // Get the input data
                let input = self.get_value_at_path(context, input_path)
                    .ok_or_else(|| RuleError::InvalidPath(input_path.to_string()))?
                    .clone();
                
                // Run the transformation
                let result = transformation.transform(input).await
                    .map_err(|e| RuleError::ActionExecutionError(format!("Transformation error: {}", e)))?;
            ...
            RuleAction::ExecuteCommand { command, args, working_dir: _working_dir } => {
            ...
            RuleAction::CallApi { url, method, headers: _headers, body: _body, response_path: _response_path } => {
                // Log the API call
            ...
            RuleAction::NotifyUser { title, message, level } => {
            ...
            RuleAction::Custom { id, config: _config } => {
                Err(RuleError::ActionExecutionError(
                    format!("Custom action '{}' not supported", id)
                ))
            },
        }
    }
        ...
        for (i, part) in parts.iter().enumerate() {
            if i == parts.len() - 1 {
                ...
                if let Value::Object(obj) = current {
                    ...
                    return Err(RuleError::InvalidPath(format!("Cannot set value at path '{}', parent is not an object", path)));
                }
            }
            ...
            if let Ok(index) = part.parse::<usize>() {
                if let Value::Array(arr) = current {
                    if index >= arr.len() {
                        return Err(RuleError::InvalidPath(format!("Array index out of bounds: {}", index)));
                    }
                    current = &mut arr[index];
                } else {
                    return Err(RuleError::InvalidPath(format!("Expected array at '{}', got {}", part, current)));
                }
            } else {
                ...
                if let Value::Object(obj) = current {
                    ...
                    return Err(RuleError::InvalidPath(format!("Expected object at '{}', got {}", part, current)));
                }
            }
        }
...
#[allow(dead_code)] // Kept for future implementation
pub async fn execute_actions(actions: &[RuleAction], context: &Value, plugin_manager: Arc<RulePluginManager>) -> Result<ActionResult> {
...
pub async fn execute_conditional_action(
    ...
    let result = evaluator.evaluate_condition(condition, &context_clone).await?;
...
impl Default for ActionExecutor {
    fn default() -> Self {
        ...
        Self::new(Arc::new(RulePluginManager::new(Arc::new(DummyPluginManager::default()))))
    }
} 