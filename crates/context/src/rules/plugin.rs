...
impl RulePluginManager {
    ...
    pub async fn get_transformation(&self, id: &str) -> Result<Arc<dyn ContextTransformation>> {
        self.core_plugin_manager.get_transformation(id).await
            .map_err(|e| RuleError::PluginError(format!("Failed to get transformation '{}': {}", id, e)))
    }
    
    /// Get an adapter by ID
    pub async fn get_adapter(&self, id: &str) -> Result<Arc<dyn ContextAdapter>> {
        self.core_plugin_manager.get_adapter(id).await
