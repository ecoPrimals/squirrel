...
impl RuleDirectoryManager {
    ...
    pub async fn ensure_directory<P: AsRef<Path>>(path: P) -> Result<()> {
        ...
        if !path.exists() {
            ...
            return Err(RuleError::DirectoryError(
    ...
    pub async fn list_rule_files(&self, category: Option<&str>) -> Result<Vec<PathBuf>> {
        ...
        while let Some(dir) = dirs.pop() {
            let mut entries = fs::read_dir(&dir)
                .await
                .map_err(|e| {
                    RuleError::DirectoryError(format!("Failed to read directory: {}", e))
                })?;
    ...
    pub fn rule_path(&self, category: &str, rule_id: &str) -> PathBuf {
        self.category_path(category).join(format!("{}.mdc", rule_id))
    }
    ...
    /// Create a rule file
    pub async fn create_rule_file(&self, category: &str, rule_id: &str, content: &str) -> Result<PathBuf> {
        let category_path = self.create_category(category).await?;
        let rule_path = category_path.join(format!("{}.mdc", rule_id));
        ...
        // Write the content
        fs::write(&rule_path, content).await.map_err(|e| {
            RuleError::DirectoryError(format!("Failed to write rule file: {}", e))
        })?;
    ...
    pub async fn read_rule_file(&self, category: &str, id: &str) -> Result<String> {
        ...
        fs::read_to_string(&rule_path)
            .await
            .map_err(|e| {
                RuleError::DirectoryError(format!("Failed to read rule file: {}", e))
            })
    }
    ...
    pub async fn update_rule_file(&self, category: &str, id: &str, content: &str) -> Result<()> {
        ...
        if let Some(parent) = rule_path.parent() {
            fs::create_dir_all(parent)
                .await
                .map_err(|e| RuleError::DirectoryError(format!("Failed to create directory: {}", e)))?;
        }
