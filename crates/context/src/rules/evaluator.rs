...
impl RuleEvaluator {
    ...
    pub async fn find_matching_rules(&self, rules: &[Arc<Rule>], context: &Value) -> Result<Vec<Arc<Rule>>> {
    ...
    pub async fn evaluate_condition(&self, condition: &RuleCondition, context: &Value) -> Result<bool> {
        match condition {
            RuleCondition::Exists { path } => {
                self.path_exists(path, context)
            },
        ...
        let regex = {
            ...
            if !cache.contains_key(pattern) {
                let regex = Regex::new(pattern)
                    .map_err(|e| RuleError::EvaluationError(format!("Invalid regex pattern: {}", e)))?;
                cache.insert(pattern.to_string(), regex);
            }
            
            cache.get(pattern).unwrap().clone()
        };
    ...
    fn compare_values(&self, value1: &Value, value2: &Value, operator: &str) -> Result<bool> {
        match operator {
            ...
            _ => Err(RuleError::EvaluationError(format!("Unknown operator: {}", operator))),
        }
    }
        ...
        match value {
            Value::Number(n) => {
                if let Some(num) = n.as_f64() {
                    ...
                    Err(RuleError::EvaluationError("Failed to convert number to f64".to_string()))
                }
            },
            Value::String(s) => {
                s.parse::<f64>()
                    .map_err(|_| RuleError::EvaluationError(format!("Failed to parse string as number: {}", s)))
            },
            Value::Bool(b) => {
                Ok(if *b { 1.0 } else { 0.0 })
            },
            _ => {
                Err(RuleError::EvaluationError("Cannot convert value to number".to_string()))
            }
        }
    }
}
...
#[allow(dead_code)] // Kept for future implementation
async fn evaluate_and_condition(evaluator: &RuleEvaluator, conditions: &[RuleCondition], context: &Value) -> Result<bool> {
...
#[allow(dead_code)] // Kept for future implementation
async fn evaluate_or_condition(evaluator: &RuleEvaluator, conditions: &[RuleCondition], context: &Value) -> Result<bool> {
    for condition in conditions {
        if evaluator.evaluate_condition(condition, context).await? {
            return Ok(true);
        }
...
#[allow(dead_code)] // Kept for future implementation
async fn evaluate_not_condition(evaluator: &RuleEvaluator, condition: &RuleCondition, context: &Value) -> Result<bool> {
        ...
        match current {
            ...
            Value::Array(arr) => {
                ...
                if let Ok(index) = part.parse::<usize>() {
                    ...
                    return Err(RuleError::InvalidPath(format!("Invalid array index: {}", part)));
                }
            },
            _ => {
                return Err(RuleError::InvalidPath(format!("Cannot navigate through non-object value at path segment: {}", part)));
            }
        }
    }
