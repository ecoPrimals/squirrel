...
pub type CommandHandler = Arc<dyn Fn(Value) -> futures::future::BoxFuture<'static, Result<Value>> + Send + Sync>;
...
impl Command {
    ...
    pub fn new(
        ...
        handler: impl Fn(Value) -> futures::future::BoxFuture<'static, Result<Value>> + Send + Sync + 'static,
    ) -> Self {
        Self {
            metadata,
