...
impl RepositoryProvider for HttpRepositoryProvider {
    async fn get_info(&self) -> Result<RepositoryInfo> {
        ...
        let info = self.client.get(&url).send().await?
            .json::<RepositoryInfo>().await?;
        Ok(info)
    }
    
    async fn list_plugins(&self) -> Result<Vec<PluginPackageInfo>> {
        let url = format!("{}/plugins", self.base_url);
        let plugins = self.client.get(&url).send().await?
            .json::<Vec<PluginPackageInfo>>().await?;
        Ok(plugins)
    }
...
impl RepositoryManager {
    ...
    /// Add a repository
    pub async fn add_repository(&self, id: &str, provider: Arc<dyn RepositoryProvider>) -> Result<()> {
        // Get repository info
    ...
    pub async fn get_plugin(&self, repo_id: &str, plugin_id: &Uuid) -> Result<Option<PluginPackageInfo>> {
        let repositories = self.repositories.read().await;
    ...
    pub async fn download_plugin(&self, repo_id: &str, plugin_id: &Uuid) -> Result<PathBuf> {
        ...
        if let Some(provider) = repositories.get(repo_id) {
            ...
            let output_path = self.download_dir.join(format!("{}.zip", plugin_id));
