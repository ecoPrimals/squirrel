...
use crate::web::{WebPlugin, WebEndpoint, WebComponent, WebRequest, WebResponse, HttpMethod, Route};
...
impl WebPluginRegistry {
    ...
    fn try_get_web_plugin<'a>(&self, plugin: &'a Arc<dyn crate::plugin::Plugin>) -> Option<&'a dyn WebPlugin> {
        // Try ExampleWebPlugin
        if let Some(ex_plugin) = plugin.as_any().downcast_ref::<crate::web::ExampleWebPlugin>() {
    ...
    pub async fn find_plugin_for_endpoint(&self, path: &str, method: HttpMethod) -> Result<Arc<dyn WebPlugin>> {
        // Get all web plugins
        ...
        Err(WebPluginError::EndpointNotFound(
            path.to_string(), 
            method.to_string()
        ).into())
    }
    ...
    fn supports_endpoint(&self, plugin: &Arc<dyn WebPlugin>, path: &str, method: HttpMethod) -> bool {
        for endpoint in plugin.get_endpoints() {
    ...
    pub async fn find_plugin_for_component(&self, component_id: &Uuid) -> Result<Arc<dyn WebPlugin>> {
        // Get all web plugins
        let web_plugins = self.get_web_plugins().await;
    ...
    pub async fn find_plugin_for_path(&self, path: &str, method: HttpMethod) -> Result<(Arc<dyn WebPlugin>, Option<HashMap<String, String>>)> {
    ...
    pub async fn handle_request(&self, mut request: WebRequest) -> Result<WebResponse> {
        ...
        let result = self.find_plugin_for_endpoint(&request.path, request.method.clone()).await;
        ...
        let path_result = self.find_plugin_for_path(&request.path, request.method.clone()).await;
        ...
        Err(WebPluginError::EndpointNotFound(request.path.clone(), request.method.to_string()).into())
    }
