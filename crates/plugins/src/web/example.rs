...
use crate::web::adapter::{LegacyWebPluginTrait, LegacyWebComponent};
...
impl ExampleWebPlugin {
    ...
    async fn handle_get_example(&self, id: &str) -> Result<WebResponse> {
        ...
        if let Some(item) = data.get(id) {
            ...
            Ok(WebResponse::not_found(&format!("Example with ID {} not found", id)))
        }
    }
...
impl Plugin for ExampleWebPlugin {
    ...
    async fn initialize(&self) -> Result<()> {
        ...
        let mut data = self.data.write().await;
        data.insert("example1".to_string(), json!({
            "id": "example1",
            "name": "Example 1",
        ...
        data.insert("example2".to_string(), json!({
            "id": "example2",
            "name": "Example 2",
...
impl WebPlugin for ExampleWebPlugin {
    ...
    async fn handle_request(&self, request: WebRequest) -> Result<WebResponse> {
        match (request.method, request.path.as_str()) {
            (HttpMethod::Get, "/api/examples") => {
                self.handle_get_examples().await
            },
...
impl LegacyWebPluginTrait for ExampleWebPlugin {
    fn get_endpoints(&self) -> Vec<crate::plugin::WebEndpoint> {
        ...
        WebPlugin::get_endpoints(self).iter().map(|endpoint| {
    ...
    fn get_components(&self) -> Vec<LegacyWebComponent> {
        ...
        WebPlugin::get_components(self).iter().map(|component| {
            LegacyWebComponent {
