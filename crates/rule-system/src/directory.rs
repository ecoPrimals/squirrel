...
impl Default for RuleDirectoryConfig {
    fn default() -> Self {
        Self {
            ...
            include_patterns: vec!["**/*.mdc".to_string(), "**/*.yaml".to_string(), "**/*.yml".to_string()],
...
impl RuleDirectoryManager {
    ...
    pub async fn initialize(&self) -> RuleSystemResult<()> {
        ...
        sources.insert("default".to_string(), RuleSourceMetadata {
    ...
    pub async fn add_source(&self, id: impl Into<String>, metadata: RuleSourceMetadata) -> RuleSystemResult<()> {
        let id = id.into();
        let mut sources = self.sources.write().await;
        
        // Check if source already exists
        if sources.contains_key(&id) {
            return Err(RuleSystemError::Other(format!("Source already exists: {}", id)));
        }
        
        // Check if directory exists
        if !metadata.directory.exists() {
            return Err(RuleSystemError::DirectoryNotFound(metadata.directory.clone()));
        }
    ...
    pub async fn create_rule_file(&self, id: impl AsRef<str>, category: Option<impl AsRef<str>>, content: impl AsRef<str>) -> RuleSystemResult<PathBuf> {
        let config = self.config.read().await;
        let root_dir = &config.root_directory;
    ...
    pub async fn rule_file_exists(&self, id: impl AsRef<str>, category: Option<impl AsRef<str>>) -> RuleSystemResult<bool> {
        let config = self.config.read().await;
        let root_dir = &config.root_directory;
        
        // Determine the file path
        let file_path = if let Some(category) = category {
            root_dir.join(category.as_ref()).join(format!("{}.{}", id.as_ref(), config.default_extension))
        } else {
            root_dir.join(format!("{}.{}", id.as_ref(), config.default_extension))
        };
    ...
    pub async fn delete_rule_file(&self, id: impl AsRef<str>, category: Option<impl AsRef<str>>) -> RuleSystemResult<()> {
        let config = self.config.read().await;
        let root_dir = &config.root_directory;
...
/// Creates a new rule directory manager with the given configuration
pub fn create_rule_directory_manager_with_config(config: RuleDirectoryConfig) -> RuleDirectoryManager {
    RuleDirectoryManagerFactory::create_manager_with_config(config)
}

/// Creates a new rule directory manager with a custom root directory
pub fn create_rule_directory_manager_with_root_dir(root_dir: impl Into<PathBuf>) -> RuleDirectoryManager {
...
pub trait FileWatcher: Send + Sync {
    ...
    async fn watch_directory(&self, directory: impl AsRef<Path> + Send) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    
    /// Stop watching a directory
    async fn stop_watching(&self, directory: impl AsRef<Path> + Send) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
