...
use crate::error::{RuleParserError, RuleSystemResult, RuleSystemError};
...
impl RuleParser {
    ...
    pub async fn parse_rule_file(&self, path: impl AsRef<Path>) -> RuleSystemResult<Rule> {
        ...
        // Parse the rule
        let mut rule = self.parse_rule(&content)
    ...
    pub fn parse_frontmatter<'a>(&self, content: &'a str) -> Result<(HashMap<String, Value>, &'a str), RuleParserError> {
    ...
    fn detect_frontmatter(&self, content: &str) -> Result<(FrontMatterFormat, usize, usize), RuleParserError> {
        // YAML frontmatter (---...---)
        ...
        Err(RuleParserError::InvalidFrontmatter("No valid frontmatter found".to_string()))
    }
    ...
    fn parse_yaml_frontmatter(&self, content: &str) -> Result<HashMap<String, Value>, RuleParserError> {
        serde_yaml::from_str(content)
            .map_err(|e| RuleParserError::YamlError(e.to_string()))
    }
    ...
    fn parse_toml_frontmatter(&self, content: &str) -> Result<HashMap<String, Value>, RuleParserError> {
        let value: toml::Value = toml::from_str(content)
            .map_err(|e| RuleParserError::TomlError(e.to_string()))?;
        
        // Convert TOML value to serde_json::Value
        let json_value = serde_json::to_value(value)
            .map_err(|e| RuleParserError::JsonError(e.to_string()))?;
        ...
        if let Value::Object(map) = json_value {
            ...
            Err(RuleParserError::TomlError("Root is not a table".to_string()))
        }
    }
    ...
    fn parse_json_frontmatter(&self, content: &str) -> Result<HashMap<String, Value>, RuleParserError> {
        ...
        if let Value::Object(map) = value {
            ...
            Err(RuleParserError::JsonError("Root is not an object".to_string()))
        }
    }
    ...
    fn parse_sections(&self, body: &str) -> Result<Vec<RuleSection>, RuleParserError> {
        ...
        for line in body.lines() {
            ...
            if let Some(captures) = section_regex.captures(line) {
                ...
                // Start new section
                let section_name = captures.get(1).unwrap().as_str().trim().to_string();
    ...
    fn create_rule(&self, frontmatter: HashMap<String, Value>, sections: Vec<RuleSection>) -> Result<Rule, RuleParserError> {
        // Extract required fields from frontmatter
        let id = self.extract_string_field(&frontmatter, "id")
                ...
                _ => {
                    ...
                    if self.config.extract_metadata {
                        rule.metadata.insert(
                            section.name.clone(),
                            Value::String(section.content.clone()),
                        );
                    }
                }
            }
    ...
    fn extract_string_field(&self, map: &HashMap<String, Value>, key: &str) -> Result<String, RuleParserError> {
        map.get(key)
    ...
    fn extract_integer_field(&self, map: &HashMap<String, Value>, key: &str) -> Result<i32, RuleParserError> {
        map.get(key)
            .ok_or_else(|| RuleParserError::MissingField(key.to_string()))?
            .as_i64()
            .map(|i| i as i32)
    ...
    fn extract_string_array_field(&self, map: &HashMap<String, Value>, key: &str) -> Result<Vec<String>, RuleParserError> {
