...
pub trait EnhancedRecoveryHandler: fmt::Debug + Send + Sync {
    ...
    fn handle_action<'a>(
        ...
        error: &'a ToolError,
        tool_manager: &'a dyn ToolManager,
...
impl EnhancedRecoveryHook {
    ...
    pub async fn apply_recovery_action(
        ...
        let prev_state = match tool_manager.get_tool(tool_id).await {
        ...
        let result = match action {
            ...
            AdvancedRecoveryAction::Recreate => {
                if let Ok(Some(_tool)) = tool_manager.get_tool(tool_id).await {
            ...
            AdvancedRecoveryAction::Terminate => {
        ...
        let new_state = match tool_manager.get_tool(tool_id).await {
            Ok(Some(_)) => "Active".to_string(),
            Ok(None) => "NotFound".to_string(),
            Err(_) => "Error".to_string(),
        };
        
        let success = result.as_ref().map(|r| *r).unwrap_or(false);
        let error_message = if result.is_err() { 
            Some(format!("{:?}", result.as_ref().unwrap_err()))
        } else {
            None
        };

        let recovery_attempt = EnhancedRecoveryAttempt {
            tool_id: tool_id.to_string(),
            action: action.clone(),
            success,
            duration: start.elapsed(),
            previous_state: Some("Unknown".to_string()),
            new_state: Some("Recovered".to_string()),
            error_message,
            timestamp: std::time::SystemTime::now(),
        };
        
        // Record the attempt
        self.record_attempt(recovery_attempt).await;
...
pub trait ToolManagerRecoveryExt {
    ...
    async fn get_recovery_strategy(&self, tool_id: &str) -> Result<EnhancedRecoveryStrategy, ToolError>;

    async fn set_recovery_strategy(&self, tool_id: &str, strategy: EnhancedRecoveryStrategy) -> Result<(), ToolError>;

    async fn get_recovery_history(&self, tool_id: &str) -> Result<Vec<EnhancedRecoveryAttempt>, ToolError>;

    async fn clear_recovery_history(&self, tool_id: &str) -> Result<(), ToolError>;
...
impl<T: ToolManager> ToolManagerRecoveryExt for T {
    /// Perform enhanced recovery for a tool
    fn perform_enhanced_recovery<'a>(
        &'a self,
        tool_id: &'a str,
    ...
    async fn get_recovery_strategy(&self, _tool_id: &str) -> Result<EnhancedRecoveryStrategy, ToolError> {
        // Default strategy with proper struct variant syntax
        Ok(EnhancedRecoveryStrategy::default())
    }
