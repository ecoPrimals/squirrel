...
use crate::protocol::types::{
    MCPMessage, MessageType, ProtocolVersion, RequestId, SessionId, ClientId,
    SecurityMetadata, // Using SecurityMetadata from types module
};
use crate::types::{MCPResponse, ProtocolState};
use crate::types::ProtocolState as CoreProtocolState;
// Security validation delegated to BearDog framework
...
impl MCPProtocol for MCPProtocolImpl {
    ...
    async fn get_state(&self) -> Result<CoreProtocolState, crate::error::Error> {
        // Get the current protocol state from the base
        let protocol_state = self.base.get_protocol_state();
        
        // Convert protocol state to core protocol state
        let core_state = match protocol_state {
            ProtocolState::Initializing => CoreProtocolState {
                initialized: false,
                ready: false,
                error: None,
                version: self.base.get_config().version.clone(),
                features: vec!["initializing".to_string()],
                connections: 0,
                last_activity: chrono::Utc::now(),
                metrics: std::collections::HashMap::new(),
            },
            ProtocolState::Ready => CoreProtocolState {
                initialized: true,
                ready: true,
                error: None,
                version: self.base.get_config().version.clone(),
                features: vec!["ready".to_string(), "messaging".to_string(), "tools".to_string()],
                connections: self.get_active_connections().await.unwrap_or(0),
                last_activity: chrono::Utc::now(),
                metrics: self.collect_metrics().await,
            },
            ProtocolState::Error => CoreProtocolState {
                initialized: false,
                ready: false,
                error: Some("Protocol in error state".to_string()),
                version: self.base.get_config().version.clone(),
                features: vec!["error".to_string()],
                connections: 0,
                last_activity: chrono::Utc::now(),
                metrics: std::collections::HashMap::new(),
            },
            ProtocolState::Disconnected => CoreProtocolState {
                initialized: false,
                ready: false,
                error: None,
                version: self.base.get_config().version.clone(),
                features: vec!["disconnected".to_string()],
                connections: 0,
                last_activity: chrono::Utc::now(),
                metrics: std::collections::HashMap::new(),
            },
    ...
    async fn get_active_connections(&self) -> Result<u32, crate::error::Error> {
    ...
    async fn collect_metrics(&self) -> std::collections::HashMap<String, f64> {
        ...
        metrics.insert("errors_encountered".to_string(), 0.0);
        metrics.insert("average_response_time_ms".to_string(), 0.0);
