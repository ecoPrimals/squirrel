...
impl ProductionError {
    ...
    pub fn is_recoverable(&self) -> bool {
        match self {
            ...
            ProductionError::Serialization { recovery_possible, .. } => *recovery_possible,
    ...
    pub fn retry_delay_ms(&self) -> Option<u64> {
        match self {
            ...
            ProductionError::Concurrency { retry_possible, .. } => {
                if *retry_possible { Some(100) } else { None }
            }
            ProductionError::Timeout { retry_possible, .. } => {
                if *retry_possible { Some(1000) } else { None }
            }
            _ => None,
        }
    }
    ...
    pub fn severity(&self) -> ErrorSeverity {
        match self {
            ...
            ProductionError::ServiceUnavailable { fallback_available, .. } => {
                if *fallback_available { ErrorSeverity::Medium } else { ErrorSeverity::High }
            }
            ProductionError::Database { connection_available, .. } => {
                if *connection_available { ErrorSeverity::Medium } else { ErrorSeverity::High }
            }
