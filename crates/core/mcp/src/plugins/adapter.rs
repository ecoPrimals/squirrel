...
impl ToolPluginAdapter {
    ...
    pub fn new(tool_id: String, tool_manager: Arc<dyn ToolManager>) -> Self {
        Self {
            metadata: PluginMetadata {
    ...
    async fn get_tool(&self) -> Result<Tool> {
        let tool_info = self.tool_manager.get_tool(&self.tool_id).await
            .map_err(|e| anyhow!("Failed to get tool info: {}", e))?;

        match tool_info {
            Some(info) => {
                // Convert ToolInfo to Tool (simplified)
                let tool = Tool {
                    name: info.name,
...
impl ToolPluginFactory {
    ...
    pub const fn new(tool_manager: Arc<dyn ToolManager>) -> Self {
        Self {
            tool_manager,
        }
    }
    ...
    pub async fn create_plugin_adapter(&self, tool_id: &str) -> Result<ToolPluginAdapter> {
        ...
        match tool_info {
            ...
            None => Err(anyhow!("Tool not found: {}", tool_id)),
        }
    }
