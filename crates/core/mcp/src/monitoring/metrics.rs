...
impl MetricsCollector {
    ...
    pub fn create_performance_snapshot(&self) -> PerformanceSnapshot {
        ...
        let metrics_result = SafeOperation::execute(|| {
            self.metrics.read()
                .map_err(|e| ProductionError::concurrency(
                    format!("Failed to acquire metrics read lock: {}", e),
                    "metrics_snapshot",
                    true,
                ))
        });
        
        let metrics_guard = match metrics_result.result() {
            Ok(guard) => guard,
            Err(e) => {
                error!("Failed to acquire metrics read lock for snapshot: {}", e);
                // Return a safe default snapshot if lock fails
        ...
        let get_float = |name: &str| -> f64 {
            SafeOperation::execute(|| {
                get_metric_value(name)
                    ...
                    .ok_or_else(|| ProductionError::serialization(
                        format!("Failed to extract float value for metric '{}'", name),
                        "metric_value",
                        true,
                    ))
            }).unwrap_or(0.0)
        ...
        let get_int = |name: &str| -> i64 {
            SafeOperation::execute(|| {
                get_metric_value(name)
                    ...
                    .ok_or_else(|| ProductionError::serialization(
                        format!("Failed to extract integer value for metric '{}'", name),
                        "metric_value",
                        true,
                    ))
            }).unwrap_or(0)
        ...
        let memory_usage_bytes = SafeOperation::execute(|| {
            self.collect_memory_usage()
                .ok_or_else(|| ProductionError::resource_exhausted(
                    "memory_usage",
                    "Failed to collect memory usage",
                    None,
                    None,
                ))
        }).unwrap_or(0u64);

        // Collect CPU usage safely
        let _cpu_usage_percent = SafeOperation::execute(|| {
            self.collect_cpu_usage()
                .ok_or_else(|| ProductionError::resource_exhausted(
                    "cpu_usage", 
                    "Failed to collect CPU usage",
                    None,
                    None,
                ))
        }).unwrap_or(0.0);
