...
impl CapabilityCryptoProvider {
    ...
    async fn discover_endpoint(&mut self) -> Result<Arc<str>> {
        ...
        if let Ok(endpoint) = std::env::var("CRYPTO_SIGNING_ENDPOINT") {
            info!("✅ Discovered crypto.signing via CRYPTO_SIGNING_ENDPOINT: {}", endpoint);
                ...
                if let Some(capabilities) = response.get("result").and_then(|r| r.get("capabilities")) {
                    if let Some(caps_array) = capabilities.as_array() {
                        return Ok(caps_array.iter().any(|c| {
                            c.as_str().map(|s| s == capability || s.starts_with(capability)).unwrap_or(false)
                        }));
                    }
                }
                Ok(false)
    ...
    pub async fn sign_ed25519(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        debug!("✅ Signed {} bytes via capability crypto provider", data.len());
        Ok(signature)
    }
    ...
    pub async fn verify_ed25519(
        ...
        debug!("✅ Verified signature via capability crypto provider: {}", valid);
        Ok(valid)
    }
    ...
    pub async fn verify_ed25519_with_key_id(
        ...
        debug!("✅ Verified signature (key_id={}) via capability crypto provider: {}", key_id, valid);
        Ok(valid)
    }
