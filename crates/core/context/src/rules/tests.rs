...
fn create_test_rule(id: &str) -> Rule {
    Rule {
        ...
        actions: vec![RuleAction::ModifyContext {
...
async fn test_rule_directory_manager() -> crate::rules::error::Result<()> {
    ...
    let content = "---\nid: test-rule\nname: Test Rule\n---\n# Test Rule\n\nThis is a test rule.";
    dir_manager
        .create_rule_file("test", "test-rule", content)
        .await?;
    ...
    dir_manager
        .update_rule_file("test", "test-rule", updated_content)
        .await?;
...
async fn test_rule_repository() -> crate::rules::error::Result<()> {
    ...
    assert_eq!(
        test_rules.len(),
        3,
        "Expected 3 rules in the 'test' category"
    );

    // Get rules by pattern
    let matching_pattern_rules = repository.get_rules_by_pattern("test.*").await?;
    assert_eq!(
        matching_pattern_rules.len(),
        2,
        "Expected 2 rules matching pattern 'test.*'"
    );
    ...
    assert_eq!(
        matched_rules.len(),
        3,
        "Expected 3 rules matching 'test.special'"
    );
    assert!(
        matched_rules.iter().any(|r| r.id() == "rule2"),
        "rule2 should be in the matching rules"
    );
...
async fn test_action_executor() -> crate::rules::error::Result<()> {
    ...
    action_executor
        .execute_action(&set_action, &mut context)
        .await?;
    ...
    action_executor
        .execute_action(&recovery_action, &mut context)
        .await?;
    ...
    action_executor
        .execute_action(&log_action, &mut context)
        .await?;
    ...
    action_executor
        .execute_action(&notify_action, &mut context)
        .await?;
    ...
    let result = action_executor
        .execute_action(&custom_action, &mut context)
        .await;
...
async fn test_rule_manager() -> crate::rules::error::Result<()> {
    ...
    assert_eq!(
        ...
        1,
        "One rule should have been applied"
    );
    assert_eq!(result.rules_applied[0].id, "test_rule");

    // Verify the context was modified as expected
    assert_eq!(
        context["processed_by"], "test_rule",
        "Rule action should have modified the context"
    );
