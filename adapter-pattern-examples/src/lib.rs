    ...
    async fn execute_command(&self, command: &str, args: Vec<String>) -> CommandResult<String> {
        ...
        let registry_clone = {
            let registry = self.registry.lock()
                .map_err(|e| CommandError::Internal(format!("Lock error: {}", e)))?;
            registry.list_commands().into_iter().map(|name| {
                let cmd = registry.commands.get(&name).unwrap().clone();
                (name, cmd)
...
impl McpAdapter {
    ...
    pub fn add_user(&mut self, username: &str, password: &str, roles: Vec<UserRole>) {
        self.users.insert(username.to_string(), (password.to_string(), roles));
    }
    ...
    pub fn generate_token(&mut self, username: &str, password: &str) -> CommandResult<String> {
        ...
        if let Some((stored_password, _)) = self.users.get(username) {
            if stored_password != password {
                return Err(CommandError::AuthenticationFailed("Invalid password".to_string()));
            }
        } else {
            return Err(CommandError::AuthenticationFailed("User not found".to_string()));
        }
    ...
    fn log_command(&mut self, command: &str, args: &[String], user: Option<&str>, success: bool, message: String) {
    ...
    pub async fn execute_with_auth(&mut self, command: &str, args: Vec<String>, auth: Auth) -> CommandResult<String> {
        println!("McpAdapter::execute_with_auth called for {} with {:?}", command, auth);
        ...
        let username_string = match &auth {
            ...
            Auth::Token(token) => {
                ...
                match tokens.get(token) {
                    ...
                    None => return Err(CommandError::AuthenticationFailed("Invalid token".to_string())),
                }
            },
            Auth::None => None,
        };
        ...
        match auth {
            Auth::User(username, password) => {
                ...
                if let Some((stored_password, roles)) = self.users.get(&username) {
                    ...
                    let filtered = all_commands.into_iter().filter(|cmd| {
                    ...
                    Err(CommandError::AuthenticationFailed("User not found".to_string()))
                }
            },
            Auth::Token(token) => {
                // Verify token
                if let Some(username) = self.tokens.get(&token) {
                    ...
                    Err(CommandError::AuthenticationFailed("Invalid token".to_string()))
                }
            },
            Auth::None => {
                // Filter out commands that require authentication
                let filtered = all_commands.into_iter().filter(|cmd| {
                    !self.command_permissions.contains_key(cmd)
                }).collect();
...
mod tests {
    ...
    async fn test_registry_adapter() -> CommandResult<()> {
        ...
        let result = adapter.execute_command("echo", vec!["Hello".to_string(), "there!".to_string()]).await?;
    ...
    async fn test_mcp_adapter_authentication() -> CommandResult<()> {
        ...
        let result = adapter.execute_with_auth("admin-cmd", vec![], token_auth).await?;
        assert_eq!(result, "Admin data");
    ...
    async fn test_plugin_adapter() -> CommandResult<()> {
        ...
        // Execute command
        let result = adapter.execute_command("plugin-cmd", vec!["arg1".to_string(), "arg2".to_string()]).await?;
    ...
    async fn test_polymorphic_adapter_usage() -> CommandResult<()> {
        ...
        async fn execute_with_adapter(adapter: &dyn CommandAdapter, command: &str) -> CommandResult<String> {
