...
impl CalculatorCommand {
    ...
    fn calculate(&self, operation: &str, x: f64, y: f64) -> CommandResult<f64> {
        match operation {
            ...
            "div" => {
                if y == 0.0 {
                    Err(CommandError::ExecutionFailed("Division by zero".to_string()))
                } else {
                    Ok(x / y)
                }
            },
            _ => Err(CommandError::ExecutionFailed(format!("Unknown operation: {}", operation))),
        }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add operation
    let add_result = adapter.execute_command(
        "calc", 
    ...
    // Multiply operation
    let mul_result = adapter.execute_command(
        "calc", 
    ...
    // Division operation
    let div_result = adapter.execute_command(
        "calc", 
    ...
    let div_zero = adapter.execute_command(
        "calc", 
    ...
    let london_weather = adapter.execute_command(
        "weather", 
        vec!["London".to_string()]
    ).await?;
    println!("{}", london_weather);
    
    let tokyo_weather = adapter.execute_command(
        "weather", 
        vec!["Tokyo".to_string()]
    ).await?;
