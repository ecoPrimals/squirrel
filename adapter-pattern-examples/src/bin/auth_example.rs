use adapter_pattern_examples::{
    ...
    McpAdapter, Auth, UserRole, TestCommand
};
...
impl Command for AdminCommand {
    ...
    async fn execute(&self, args: Vec<String>) -> CommandResult<String> {
        if args.is_empty() {
            return Ok("Current configuration: debug=false, verbose=false, timeout=30s".to_string());
        }
        ...
        match setting.as_str() {
            ...
            _ => Err(CommandError::ExecutionFailed(format!("Unknown setting: {}", setting))),
        }
    }
}
    ...
    async fn execute(&self, args: Vec<String>) -> CommandResult<String> {
        ...
        match action.as_str() {
            ...
            "update" => {
                ...
                Ok(format!("Updated profile field '{}' to '{}'", args[1], args[2]))
            },
            "delete" => Ok("Profile deleted. This cannot be undone.".to_string()),
            _ => Err(CommandError::ExecutionFailed(format!("Unknown profile action: {}", action))),
        }
    }
}
...
impl Command for PublicInfoCommand {
    ...
    async fn execute(&self, args: Vec<String>) -> CommandResult<String> {
        ...
        match topic.as_str() {
            ...
            _ => Err(CommandError::ExecutionFailed(format!("Unknown info topic: {}", topic))),
        }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    mcp_adapter.register_command(Arc::new(TestCommand::new("test", "Test command", "Test result")))?;
    ...
    loop {
        ...
        let available_commands = mcp_adapter.get_available_commands(current_auth.clone()).await?;
        ...
        match input.as_str() {
            ...
            _ => {
                ...
                match mcp_adapter.execute_with_auth(&command, args, current_auth.clone()).await {
