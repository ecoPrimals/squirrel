use adapter_pattern_examples::{
    ...
    RegistryAdapter, McpAdapter, PluginAdapter, Auth, UserRole
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = registry_adapter.execute_command("echo", vec!["Hello".to_string(), "from".to_string(), "adapter!".to_string()]).await?;
    ...
    let result = mcp_adapter.execute_with_auth("secure", vec![], user_auth.clone()).await?;
    println!("User access to secure command: {}", result);
    
    // Try user access to admin command (should fail)
    let result = mcp_adapter.execute_with_auth("admin-cmd", vec![], user_auth).await;
    ...
    let result = mcp_adapter.execute_with_auth("admin-cmd", vec![], admin_auth.clone()).await?;
    ...
    // Execute command
    let result = plugin_adapter.execute_command("plugin-cmd", vec!["arg1".to_string(), "arg2".to_string()]).await?;
