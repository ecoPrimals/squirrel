use adapter_pattern_examples::{
    Command, CommandAdapter, CommandResult, CommandError,
    RegistryAdapter, McpAdapter, Auth, UserRole, TestCommand
};
    ...
    async fn execute(&self, args: Vec<String>) -> CommandResult<String> {
        if args.is_empty() {
            ...
            match self.adapter.get_help(command).await {
                Ok(help) => Ok(help),
                Err(_) => Err(CommandError::NotFound(format!("Command '{}' not found", command))),
            }
        }
    }
}
    ...
    async fn execute(&self, args: Vec<String>) -> CommandResult<String> {
        ...
        match style.as_str() {
            ...
            _ => Err(CommandError::ExecutionFailed(
                format!("Unknown greeting style: {}. Use 'formal' or 'casual'.", style)
            )),
        }
    }
}
...
impl CliApp {
    ...
    async fn initialize(&mut self) -> CommandResult<()> {
        ...
        self.registry_adapter.register_command(Arc::new(version_cmd))?;
        self.registry_adapter.register_command(Arc::new(echo_cmd))?;
        self.registry_adapter.register_command(Arc::new(greet_cmd))?;
        ...
        self.registry_adapter.register_command(Arc::new(secure_cmd))?;
    ...
    async fn execute_with_auth(&self, command: &str, args: Vec<String>, auth: Auth) -> CommandResult<String> {
        println!("Debug - execute_with_auth called with command: {} and auth", command);
                ...
                Auth::User(username, password) => {
                    ...
                    if (username == "user" && password == "password") || 
                       (username == "admin" && password == "password") {
                        ...
                        return Err(CommandError::AuthenticationFailed("Invalid username or password".to_string()));
                    }
                },
                _ => return Err(CommandError::AuthenticationFailed(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_commands = app.available_commands(Auth::User("user".to_string(), "password".to_string())).await?;
    ...
    if command == "login" {
        ...
        if command_args.len() > 2 {
            ...
            match app.execute_with_auth(subcommand, subcommand_args, auth).await {
    ...
    println!("Executing command: {} with args: {:?}", command, command_args);
