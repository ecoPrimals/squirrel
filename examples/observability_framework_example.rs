...
impl RequestProcessor {
    ...
    fn new(name: impl Into<String>, framework: ObservabilityFramework) -> ObservabilityResult<Self> {
        ...
        framework.health_checker().register_check_fn(
            format!("{}_health", name_str),
            name_str.clone(),
            format!("Health check for {}", name_str),
            {
                let name_str = name_str.clone(); // Clone for the closure
                move || {
                    // Simple health check based on active requests
                    // In a real implementation, this would check more substantive metrics
                    HealthCheckResult::healthy(format!("{} is healthy", name_str))
                }
            },
        )?;
    ...
    fn simulate_health_status_change(&self, new_status: HealthStatus) -> ObservabilityResult<()> {
        ...
        let alert = self.framework.alert_manager().publish_alert(
            observability::alerting::Alert::health_status(
                &self.name,
                HealthStatus::Healthy, // Assuming previous status was healthy
                new_status,
                format!("Simulated health status change for {}", self.name),
            )
        )?;
        
        info!("Created health status alert: {}", alert.id());
        Ok(())
    }
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let framework = observability::initialize()?;
